#!/usr/bin/env python3
"""Final local audit for 2026-08-03 (do not push)."""
...
def main() -> int:
    ...
    if "August 3, 2026 — current slate" not in html:
        fail("manifest missing August 3 current slate")
    ...
    if not m or int(m.group(1)) != 56:
        fail(f"expected 56 props, got {m.group(0) if m else None}")
    ...
    if not block:
        ...
        if len(titles) != 8:
            fail(f"expected 8 games, got {len(titles)}")
        if len(metas) != 8:
            fail(f"expected 8 gameMeta, got {len(metas)}")
    ...
    elif 'content="2026-08-02"' not in arch.read_text(encoding="utf-8"):
