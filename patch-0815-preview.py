#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-15. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-08-14.html"
SHEET_DATE = "2026-08-15"

spec = importlib.util.spec_from_file_location("build0815", ROOT / "build-sheet-2026-08-15.py")
...
# 8/15 judgment: Chourio owns the most complete O0.5 lane on the board — the slate's
# most HR-prone arm (Wrobleski +1.52), the best platoon split (+1.17), top-tier park
# (+20% game, +18% RHB) and real form (1 HR / 2 near-HR) behind a 29.1 zone fit.
# Yordan Alvarez outranks him on raw zone (37.4) but has zero HR and zero near-HR, so
# the score is carrying park and matchup with no power behind it.
# Encarnacion-Strand takes O1.5 on the loudest multi-HR profile of the slate — 2 HR
# plus 4 near-HR, 25.9 zone HR, 38.7 zone barrel, +0.59 risk — in a different game.
_o05_lock = next((r for r in straight_rows if r["name_plain"] == "Jackson Chourio"), None)
_o15_lock = next(
    (r for r in straight_rows if r["name_plain"] == "Christian Encarnacion-Strand"), None
)
...
# 8/15 Goblin judgment: three separate games per ticket, every leg carrying a positive
# split AND positive opposing HR risk AND a positive hand park.
#   3 Leg  — Gary Sanchez (+1.17 split, +1.52 risk, +18% RHB park, 2 HR, 47.1% hard hit),
#            Henry Bolte (29.9 zone / 24.9 zone HR, +0.48/+0.58, +31% park),
#            Ian Happ (+0.74/+0.83, +23% SHB park, 2 HR, high blast).
#   2 Leg  — Suarez and Mayo: both heating, both positive split and risk, and neither
#            reuses a straight.
#   Fav 3  — Walker (35.0 zone, 53.6% hard hit, +23% RHB park), Alonso (32.6 zone HR,
#            +0.59 risk), Caglianone (2 HR heating, +0.26/+0.69, +10% LHB park).
#            Chourio is the O0.5 straight, so he stays off the parlay cards.
_top3_lock_names = ["Gary Sanchez", "Henry Bolte", "Ian Happ"]
_two_lock_names = ["Eugenio Suarez", "Coby Mayo"]
_fav3_lock_names = ["Jordan Walker", "Pete Alonso", "Jac Caglianone"]
...
def update_manifest():
    ...
    if "2026-08-14" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-08-14"] = {
            "date": "2026-08-14",
            "label": "August 14, 2026",
            "href": "archive/2026-08-14.html",
        }
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 15, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-14",
        "2026-08-12",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Saturday, August 15, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
