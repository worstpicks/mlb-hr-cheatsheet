#!/usr/bin/env python3
"""Patch preview sheet to 2026-06-26. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-06-25.html"
...
# 6/26 user override: lock straights to Caglianone O0.5 + Higashioka O1.5.
forced_o05 = rows_by_plain_early.get("Jac Caglianone")
forced_o15 = rows_by_plain_early.get("Kyle Higashioka")
...
# Top 5 HR tickets: zone fit + attackability + park/weather + HR form.
...
for r in rows:
    ...
    combined_rank = summary_combined_rank(
        r, park_pct_fn=effective_park_pct, attack_bonus=attack_bonus
    )
...
combined_ranked.sort(key=lambda r: (r["combined_rank"], r["hr_zone_fit"], r["score"]), reverse=True)
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "June 26, 2026 — current slate", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Friday, June 26, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
