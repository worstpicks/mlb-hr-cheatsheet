#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-09. Does not commit or push."""
...
SHEET_DATE = "2026-08-09"

spec = importlib.util.spec_from_file_location("build0809", ROOT / "build-sheet-2026-08-09.py")
...
# 8/9 judgment: O0.5 Bell vs Misiorowski (atk/zone lead, +1.20 / +21 park).
# O1.5 Jazz vs Holmes (3 HR / 3 near) — different game.
_bell = next((r for r in straight_rows if r["name_plain"] == "Josh Bell"), None)
_jazz = next((r for r in straight_rows if r["name_plain"] == "Jazz Chisholm Jr."), None)
if _bell is not None:
    straight_o05 = _bell
if _jazz is not None and _jazz["game_key"] != straight_o05["game_key"]:
    straight_o15 = _jazz
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/9 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
# Lowe / Moreno / Wells: real form + positive split; Moreno vs Wrobleski 🧤.
_top3_lock_names = ["Nathaniel Lowe", "Gabriel Moreno", "Austin Wells"]
_two_lock_names = ["Ryan Kreidler", "Mickey Moniak"]
_fav3_lock_names = ["Elly De La Cruz", "Jimmy Crooks", "Ben Rice"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 9, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-08",
        "2026-08-07",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Sunday, August 9, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
