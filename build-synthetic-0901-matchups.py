#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-09-01.

Two arms, both confirmed against the MLB Stats API:

  TOR  Spencer Arrighetti -> Spencer Miles   PropFinder updated its HR-risk export
                          to Miles but left the matchup file on Arrighetti. Miles
                          inherits the Arrighetti file's Cleveland batter rows and
                          carries his own pitcher block from 2026-08-26.

  BOS  Brayan Bello -> Jedixson Paez         The sheet's owner exported Bello as a
                          pitch-mix stand-in and named Paez as the real starter.
                          Paez takes the Bello file's Seattle batter rows but none
                          of Bello's numbers; his own book is 3.0 MLB innings, so
                          his header renders those measured rates.
"""
...
GAPS: dict[str, dict] = {
    "Spencer Miles": {
        ...
        "oteam": "CLE",
        "lineups": "sibling",
        "sibling": f"hr-matchups-TOR-at-CLE-Spencer-Arrighetti-{DATE}.csv",
        "block_from": "hr-matchups-KC-at-TOR-Spencer-Miles-2026-08-26.csv",
    },
    "Jedixson Paez": {
        "matchup": "SEA @ BOS",
        "pteam": "BOS",
        "oteam": "SEA",
        "lineups": "sibling",
        "sibling": f"hr-matchups-SEA-at-BOS-Brayan-Bello-{DATE}.csv",
        "block_from": None,
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-TOR-at-CLE-Spencer-Arrighetti-{DATE}.csv",
    f"hr-matchups-SEA-at-BOS-Brayan-Bello-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 9/1 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# Nothing is carried. Miles already has his own same-day HR-risk row, and Paez has
# none anywhere -- Bello's would be Bello's, which is the whole point of not using it.
