#!/usr/bin/env python3
"""Quick audit for 2026-06-29 sheet before push."""
...
if zc < 55:
    errors.append(f"zoneScore count {zc} < 55 props")

for needle in ("Homerun Form", "Split + Risk + Park + Form + Zone", "Worst Pickz Straights"):
    if needle not in html:
        errors.append(f"missing feature: {needle}")

if not (ROOT / "preview" / "data" / "research-2026-06-29.json").exists():
    errors.append("missing preview/data/research-2026-06-29.json")
if not (ROOT / "preview" / "data" / "park-factors-2026-06-29.json").exists():
    errors.append("missing preview/data/park-factors-2026-06-29.json")
...
spec = importlib.util.spec_from_file_location("b", ROOT / "build-sheet-2026-06-29.py")
...
if manifest["sheets"][0]["date"] != "2026-06-29":
    errors.append("manifest current slate not 2026-06-29")
if not (ROOT / "preview" / "archive" / "2026-06-28.html").exists():
    errors.append("missing archive 2026-06-28.html")

if errors:
    print("FAIL audit-0629:")
    for e in errors:
        print(" ", e)
    raise SystemExit(1)

print(
    f"OK audit-0629 — sheet-date, {EXPECTED_GAMES}/{EXPECTED_GAMES} park headers, "
