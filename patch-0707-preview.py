#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-07. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-06.html"
SHEET_DATE = "2026-07-07"

spec = importlib.util.spec_from_file_location("build0707", ROOT / "build-sheet-2026-07-07.py")
...
# 7/7: Juan Soto #1 O0.5 composite vs Lugo 🧤; James Wood best O1.5 lane vs Imai (different game).
_juan = rows_by_plain_early.get("Juan Soto")
_jwood = rows_by_plain_early.get("James Wood")
if _juan and _jwood and _juan["game_key"] != _jwood["game_key"]:
    _juan_o05_ok = _juan in straight_o05_pool(straight_rows, strict=True)
    _jwood_o15_ok = (
        _jwood["hr"] >= 2
        and _jwood["near"] >= 2
        and _jwood["split"] >= 0.15
        and not (_jwood["split"] <= 0.0 and _jwood["risk"] <= 0.0)
    )
    if _juan_o05_ok and _jwood_o15_ok:
        straight_o05 = _juan
        straight_o15 = _jwood
straight_names = {straight_o05["name"], straight_o15["name"]}
...
if len(top3) < 3:
    ...
    top3 = pick_top_n(extra_relaxed, 3, exclude_names=straight_names, max_per_game=2, max_per_team=2)
if len(top3) < 3 and len(rows) <= 16:
    thin_top3 = sorted(
        [
            r
            for r in straight_rows
...
if len(two_leg) < 2 and len(rows) <= 16:
    thin_two = sorted(
        [
            r
            for r in straight_rows
    ...
    if len(fav3) < 3:
        fav_bum_fill = sorted(
            [
                ...
                and r["name"] not in {x["name"] for x in fav3}
                and fav_lane_ok(r)
                and (r["hr"] >= 1 or r["near"] >= 1 or r["risk"] >= 1.0)
            ],
            key=lambda x: (x["straight_attack_rank"], x["hr_zone_fit"], x["score"]),
            reverse=True,
        )
        fav3.extend(
            pick_top_n(
                fav_bum_fill,
                3 - len(fav3),
                exclude_names={x["name"] for x in top3}
                | {x["name"] for x in two_leg}
                | straight_names
                | {x["name"] for x in fav3},
                max_per_game=2,
                max_per_team=2,
            )
        )
elif len(_fav3_manual) >= 1 and len(rows) <= 16:
    have = {x["name"] for x in fav3}
    for _r in _fav3_manual:
        if _r["name"] in have:
            continue
...
# 7/6: thin favorites board (most ⭐ face HR-suppressing SPs) — last-resort fill
# takes remaining ⭐ with the loudest recent form so the card stays user-⭐ only.
if len(fav3) < 3:
    _loud_fill = sorted(
        [
            r
        ...
        key=lambda x: (x["hr"] + x["near"], x["hr_zone_fit"], x["score"]),
        reverse=True,
    )
...
if len(rows) <= 16:
    ...
    if len(top5) < top5_target:
        ...
        thin_top5 = sorted(
            [r for r in rows if r["name"] not in have],
            key=lambda r: top5_hr_ticket_rank(r, park_pct_fn=effective_park_pct),
            reverse=True,
        )
        merged = top5 + pick_top_n(
            thin_top5,
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "July 7, 2026 — current slate", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Monday, July 7, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
