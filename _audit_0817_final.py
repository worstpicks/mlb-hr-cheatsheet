#!/usr/bin/env python3
"""Final local audit for 2026-08-17 (do not push)."""
...
def main() -> int:
    ...
    if "Sunday, August 16, 2026" not in html:
        fail("hero must be Sunday, August 16, 2026")
    if "Saturday, August 15, 2026 — Worst" in html:
        fail("stale 8/15 hero still on current sheet")
    if "Friday, August 16" in html or "Saturday, August 16" in html:
        fail("wrong weekday on August 16 (must be Sunday)")
    if "August 16, 2026 — current slate" not in html:
        fail("manifest missing August 16 current slate")
    if "August 15, 2026 — current slate" in html:
        fail("8/15 still labeled current")
    ...
    expected_bum = {"Ryan Johnson", "Edward Cabrera", "Jake Irvin", "Lake Bachar"}
    ...
    arch = ROOT / "preview" / "archive" / "2026-08-16.html"
    if not arch.exists():
        fail("8/15 archive missing")
    elif 'content="2026-08-16"' not in arch.read_text(encoding="utf-8"):
        fail("8/15 archive wrong sheet-date")
