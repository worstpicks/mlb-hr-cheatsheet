...
if sheet_date != "2026-06-18":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Thursday, June 18, 2026" not in text:
    errors.append("header weekday wrong: expected Thursday, June 18, 2026")
...
stale_june17_only = [
    "Matt McLain", "Heriberto Hernandez", "Ceddanne Rafaela", "Garrett Mitchell",
    "Kyle Manzardo", "Jake Bauers", "Endy Rodriguez", "TJ Rumfield", "Bryan Reynolds",
    "Tyler Callihan", "Carson Benge", "Mookie Betts", "Ketel Marte",
    "Colt Keith", "Alec Burleson", "Willi Castro", "Seiya Suzuki", "Max Scherzer",
    "Aaron Civale", "Andrew Benintendi", "Rodolfo Durán", "Jackson Merrill",
    "Fernando Tatis Jr", "Brice Matthews", "Yohendrick Pinango", "Luke Raley",
]
for stale in stale_june17_only:
    if stale in text:
        errors.append(f"stale prior-slate name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0618:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0618")
