#!/usr/bin/env python3
"""Final local audit for 2026-08-07 (do not push)."""
...
def main() -> int:
    ...
    if "Friday, August 7, 2026" not in html:
        fail("hero must be Friday, August 7, 2026")
    if "Thursday, August 6, 2026 — Worst" in html:
        fail("stale 8/6 hero still on current sheet")
    if "Thursday, August 7" in html or "Wednesday, August 7" in html:
        fail("wrong weekday on August 7 (must be Friday)")
    if "August 7, 2026 — current slate" not in html:
        fail("manifest missing August 7 current slate")
    ...
    if not m or int(m.group(1)) != 67:
        fail(f"expected 67 props, got {m.group(0) if m else None}")
    ...
    if not arch.exists():
        fail("8/6 archive missing")
    elif 'content="2026-08-06"' not in arch.read_text(encoding="utf-8"):
        fail("8/6 archive wrong sheet-date")
