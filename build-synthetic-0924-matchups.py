#!/usr/bin/env python3
"""Fill hr-matchups PropFinder did not export for 2026-09-24.

  LAA @ SEA shipped weak-spots but no matchup file for either arm. Today's zone
  lineups, HR risk and pitcher summaries cover both, so the game is rebuilt from
  those plus each hitter's latest real batted-ball form. ODDS stay N/A.

  CLE @ BOS: PropFinder built Cleveland's side around Daniel Espino, an opener (2
  starts, 7.0 batters). The owner's card names Joey Cantillo (PRIM), a 30-start
  starter. Boston's rows come from Espino's real export -- same hitters, same prop
  market, so the listed prices are kept -- with ZONE blanked: Espino throws right,
  Cantillo left. Espino's original file is kept in data/superseded-2026-09-24/.
"""
...
GAPS: dict[str, dict] = {
    "Grayson Rodriguez": {"matchup": "LAA @ SEA", "pteam": "LAA", "oteam": "SEA", "lineups": "zone"},
    "Bryan Woo": {"matchup": "LAA @ SEA", "pteam": "SEA", "oteam": "LAA", "lineups": "zone"},
    "Joey Cantillo": {
        "matchup": "CLE @ BOS", "pteam": "CLE", "oteam": "BOS", "lineups": "sibling",
        "sibling": "superseded-2026-09-24/hr-matchups-CLE-at-BOS-Daniel-Espino-2026-09-24.csv",
        "drop_zone": True, "keep_odds": True,
    },
}
...
def sibling_batter_rows(sibling: str, drop_zone: bool = False, keep_odds: bool = False) -> tuple[list[list[str]], list[str]]:
    ...
    for line in lines[hdr_i + 1 :]:
        ...
        # A doubleheader sibling's prices belong to the other game, so they go. A
        # same-game pitcher swap keeps them: the market is still "this hitter homers
        # tonight", and those are the real listed prices for it.
        if odds_i is not None and not keep_odds:
            row[odds_i] = "N/A"
...
def write_game(pitcher: str, zrows, summaries, prior_index) -> str | None:
    ...
    if spec["lineups"] == "sibling":
        rows, header = sibling_batter_rows(
            spec["sibling"], spec.get("drop_zone", False), spec.get("keep_odds", False)
        )
