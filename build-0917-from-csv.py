#!/usr/bin/env python3
"""Build 2026-09-17 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-17"
OUT = ROOT / "build-sheet-2026-09-17.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0917-matchups.py. Nothing is carried today: every arm the sheet
...
PROBABLE_OVERRIDES: dict[str, dict] = {
    # Texas: PropFinder and MLB list Tyler Alexander, the owner's card names Cody Bradford
    # (PRIM). Alexander has 4 starts at 4.5 batters apiece -- an opener, 68 games mostly
    # in relief -- while Bradford has 7 starts at 21.7 batters and the 0-5 / 4.72 line on
    # the card. Boston's bats will see Bradford for the bulk of the game, so the sheet
    # names Bradford. Both throw left, so no lane flips.
    "BOS @ TEX": {
        "side": "home",
        "from": "Alexander",
        "to": "Bradford",
...
PITCHER_HAND = {
    # 2026-09-17: Bradford replaces the opener Alexander (see PROBABLE_OVERRIDES);
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-17 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-17 sent "Alejanro Kirk", which matched nothing at
