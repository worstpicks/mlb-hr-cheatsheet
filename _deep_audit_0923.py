#!/usr/bin/env python3
"""Deep content audit for the 2026-09-23 sheet: prop coverage, markers, bums, staleness."""
...
# (name, game override, marker), generated from build-0923-from-csv.py RAW_PROPS so
...
USER_PROPS: list[tuple[str, str | None, str]] = [
    ...
    ("Daylen Lile", ANY, ""),
    ("Brady House", ANY, ""),
    ("Pete Alonso", "TOR @ BAL (G1)", ""),
    ("Samuel Basallo", "TOR @ BAL (G1)", ""),
    ("Colton Cowser", "TOR @ BAL (G1)", ""),
    ("Coby Mayo", "TOR @ BAL (G1)", ""),
    ("Sean Keys", "TOR @ BAL (G1)", ""),
    ("Kazuma Okamoto", "TOR @ BAL (G1)", ""),
    ("Victor Bericoto", ANY, ""),
    ("Grant McCray", ANY, ""),
    ("Ryan Kreidler", ANY, ""),
    ("Josh Bell", ANY, ""),
    ("Royce Lewis", ANY, ""),
    ("Kody Clemens", ANY, ""),
    ("Emmanuel Rodriguez", ANY, ""),
    ("Vladimir Guerrero Jr.", "TOR @ BAL (G2)", ""),
    ("Kazuma Okamoto", "TOR @ BAL (G2)", ""),
    ("Pete Alonso", "TOR @ BAL (G2)", ""),
    ("Coby Mayo", "TOR @ BAL (G2)", ""),
    ("Jake Mangum", ANY, ""),
    ("Bryan Reynolds", ANY, ""),
    ("Konnor Griffin", ANY, ""),
    ("Ivan Herrera", ANY, ""),
...
EXPECTED_BUMS = {'Mason Adams', 'Gerrit Cole', 'Matthew Liberatore'}
...
SUPERSEDED_PROBABLES: list[str] = ["Bryan Hudson"]  # opener; Erick Fedde is the bulk arm (PRIM)
...
def main() -> int:
    ...
    # A pitcher header must never print a rate from a book too small to carry one.
    # On 9/16 the MLB by-hand fallback printed Jared Simpson at ".000 vs LHB, .000 vs
    # RHB" off four and ten batters -- accurate, meaningless, and read by a bettor as
    # "unhittable". Past the ten-inning floor a .000 against a whole hand does not
    # happen, so seeing one in a header means the floor was skipped somewhere.
    # One pattern reads the whole lane, so either hand being .000 is caught.
    _LANE = re.compile(
        r"([A-Z][\w.'-]+(?: [A-Z][\w.'-]+)*) BAA vs LHB (\.\d{3})(?: · vs RHB (\.\d{3}))?"
    )
    for _gm in re.finditer(r'gameMeta: "(.*?)",\n', html):
        _txt = re.sub(r"<[^>]+>", "", _gm.group(1))
        for _hit in _LANE.finditer(_txt):
            if ".000" in (_hit.group(2), _hit.group(3)):
                fail(f"header prints a .000 hand lane (sample floor skipped): {_hit.group(1)}")
    ...
    # 2026-09-23 rebalance the model ended in a clamp at 58 and a third of every
