...
if sheet_date != "2026-06-17":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Wednesday, June 17, 2026" not in text:
    errors.append("header weekday wrong: expected Wednesday, June 17, 2026")
...
stale_june16_only = [
    "Bryce Eldridge", "Bryce Harper", "Bobby Witt Jr.", "Eugenio Suarez",
    "Hunter Goodman", "Ezequiel Tovar", "Colton Cowser", "Joc Pederson",
    "Francisco Alvarez", "Matt Olson", "Yordan Alvarez", "Christian Walker",
    "Jackson Chourio", "Daniel Schneemann", "Brandon Nimmo", "Mike Trout",
    "Kyle Tucker", "Junior Caminero", "Ben Rice", "Gabriel Rincones",
    "Bo Bichette", "Sal Stewart", "Pete Crow-Armstrong", "Miguel Amaya",
    "Cam Smith", "Colt Emerson", "Henry Davis", "Brandon Lowe",
]
for stale in stale_june16_only:
    if stale in text:
        errors.append(f"stale prior-slate name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0617:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0617")
