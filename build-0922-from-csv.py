#!/usr/bin/env python3
"""Build 2026-09-22 sheet from imported CSVs and user prop list."""
...
from game_row_enrich import MIN_IP_FOR_DERIVED_HR9, alias_weather_game_key
...
from contact_rating import (
    batters_faced_estimate,
    contact_profile,
    is_opener,
    projected_k_line,
)
...
DATE = "2026-09-22"
OUT = ROOT / "build-sheet-2026-09-22.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0922-matchups.py. Nothing is carried today: every arm the sheet
...
ALIASES = {
    # the scoreboard hyphenates him; the list does not
    "Hao Yu Lee": "Hao-Yu Lee",
    # Matched on its own (the key strips periods) but rendered the typed spelling on
    # a public board. Name it so the row reads the way the scoreboard does.
    "A.J Ewing": "A.J. Ewing",
    "AJ Ewing": "A.J. Ewing",
...
# Two Max Muncys are in today's exports and the list carries no team tag. The Dodger
# is batting fourth tonight; Oakland's is on their bench at slot 11, so this is the
# Dodger. Pinned rather than guessed -- an unpinned duplicate is how one of them once
# overwrote the other and vanished off the board.
...
# Tampa Bay at New York is a doubleheader and PropFinder exported all four arms under
# the one "TB @ NYY" key. Split so each game carries its own starters and its own
# batter rows; the owner's list tags every bat in it (G1) or (G2), and several players
# appear in both.
DOUBLEHEADER_SPECS: dict[str, dict[str, dict]] = {
    "TB @ NYY": {
        "TB @ NYY (G1)": {
            "away_sp_full": "Nick Martinez",
            "home_sp_full": "Carlos Rodon",
            "files": [
                f"hr-matchups-TB-at-NYY-Nick-Martinez-{DATE}.csv",
                f"hr-matchups-TB-at-NYY-Carlos-Rodon-{DATE}.csv",
            ],
        },
        "TB @ NYY (G2)": {
            "away_sp_full": "Drew Rasmussen",
            "home_sp_full": "Max Fried",
            "files": [
                f"hr-matchups-TB-at-NYY-Drew-Rasmussen-{DATE}.csv",
                f"hr-matchups-TB-at-NYY-Max-Fried-{DATE}.csv",
            ],
        },
    },
}
...
PITCHER_HAND = {
    # 2026-09-22: Bradford replaces the opener Alexander (see PROBABLE_OVERRIDES);
    # Pecko is Houston's arm while MLB still lists TBD.
    "Cody Bradford": "L",
    "Ethan Pecko": "R",
    # Burke replaces Newcomb for Chicago (see PROBABLE_OVERRIDES). He throws right;
    # Newcomb throws left, so leaving this to the export would score every Cleveland
    # bat against the wrong hand.
    "Sean Burke": "R",
    # PropFinder exports him as "Zach"; MLB has "Zac Thornton" (id 804267), so the
    # probables lookup missed and every New York bat was being scored against a
    # right-hander. He throws left.
    "Zach Thornton": "L",
    "Zac Thornton": "L",
...
def opposing_sp_throws(gm: dict, chip: str) -> str:
    """L/R for the arm a row's chip names, read off the game's own probables.

    Switch hitters bat opposite the arm, so the split lane cannot be chosen without
    this. Match the chip (usually a surname) against both starters in the game.
    """
    key = name_lookup_key(chip)
    for full in (gm["away_sp_full"], gm["home_sp_full"]):
        fkey = name_lookup_key(full)
        if key == fkey or fkey.endswith(key) or key.endswith(name_lookup_key(full.split()[-1])):
            return pitcher_hand_label(full)
    return pitcher_hand_label(chip)


def _within_one_edit(a: str, b: str) -> bool:
    """True when a and b differ by one inserted, deleted, changed or SWAPPED letter.

    A transposition is one keystroke gone astray, the same as the other three, but
    it scores as two substitutions and so used to fall through: the owner typed
    "Pasuqantino" for Pasquantino and the prop dropped off the sheet entirely.
    Still bounded to a single slip, and still only accepted when exactly one export
    name is that close.
    """
    if a == b:
        return True
    ...
    if len(a) == len(b):
        ...
        if len(diff) == 1:
            return True
        # adjacent pair swapped and nothing else
...
def load_batter_profiles() -> dict[str, dict]:
    ...
    # The feed and the prop list disagree about first names: Savant had "Leo Bernal"
    # while the board says "Leonardo Bernal", and the exact-key lookup came back
    # empty -- which reads downstream as "this hitter has no strikeout data" rather
    # than "we failed to find him", and quietly cost him his contact stars.
    #
    # So file a second key on first-initial + surname. It is only added when exactly
...
def load_lineup_order() -> dict[tuple[str, str], list[dict]]:
    ...
    # A doubleheader is two games under one matchup name, and the sheet keys them
    # "(G1)"/"(G2)". Keying both by the bare name let the second overwrite the first,
    # and then neither matched the sheet -- so both games silently lost their projected
    # K line while the other fourteen had one.
    from research.mlb_api import dh_aware_keys

    games_list = data.get("games", [])
    for key, game in zip(dh_aware_keys(games_list), games_list):
...
def load_probable_ids(sheet_date: str) -> dict[str, int]:
    """Probable's full name -> MLB id, straight off today's schedule.

    Searching by name is ambiguous: two pitchers are named Jack Perkins, the search
    returns both, and the caller -- unable to tell them apart -- takes neither and
    falls back to a league figure. The schedule already names tonight's arm exactly,
    so resolve the id there and only guess when he is not on it.
    """
    import urllib.request
        ...
        for day in sched.get("dates", []):
            for game in day.get("games", []):
                for side in ("away", "home"):
                    pp = (game.get("teams", {}).get(side) or {}).get("probablePitcher")
                    if pp and pp.get("fullName") and pp.get("id"):
                        out[pp["fullName"]] = int(pp["id"])
    except Exception:
        pass
    return out
...
def load_pitcher_workload(games_csv: dict[str, dict]) -> dict[str, float]:
    ...
    names.discard(None)
    probable_ids = load_probable_ids(DATE)
    folded_ids = {name_lookup_key(n): i for n, i in probable_ids.items()}
    for full in sorted(names):
        try:
            pid = probable_ids.get(full) or folded_ids.get(name_lookup_key(full))
            if pid is None:
                ...
                if len(people) != 1:
                    print(f"  WARN {full}: not on today's schedule and the name is "
                          f"ambiguous ({len(people)} pitchers); using the league figure")
                    continue
                pid = people[0]["id"]
            ...
            if bf and gs:
                per_start = bf / gs
                # Only the HIGH side is bad data. That guard exists because season
                # totals mixing relief work inflated Bello to 38.9 batters a start;
                # a starter-only split simply cannot legitimately exceed ~32.
                #
                # A LOW figure is not bad data, it is a different job. Braydon Fisher
                # has nine starts averaging 4.4 batters because he is an opener, and
                # substituting the league start for him projected a pitcher who faces
                # five men as though he faces twenty-two -- roughly four times the
                # strikeouts. Take the measured figure and let the estimator regress
                # it toward the opener prior instead.
                if per_start > 32.0:
                    print(f"  WARN {full}: {per_start:.1f} batters per start is too high "
                          "for a starter-only split; using the league figure")
                    continue
                if is_opener(bf, gs):
                    print(f"  {full}: opener — {per_start:.1f} batters over {gs} starts, "
                          "projected on the opener lane rather than the league start")
...
def _pct_or_none(text: str | None) -> int | None:
    """Percent cell, where BLANK means unknown rather than zero.

    The stadium-only and weather-only columns are the split behind the park
    number. A blank one is missing data; reporting it as 0% would tell the reader
    the park effect is entirely the other half, which is a claim the data does not
    make. format_park_segment already drops the breakdown when these are None, so
    the honest answer is to let them be None.
    """
    ...
    with path.open(encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            entry = {
                ...
                "hr_weather": _pct_or_none(row.get("HR % Weather")),
                "hr_stadium": _pct_or_none(row.get("HR % Stadium")),
            }
...
_MLB_HAND_LANE_CACHE: dict[str, dict | None] = {}


def _ip_to_float(text) -> float | None:
    """Baseball innings ("89.2" = 89 innings and two outs) as a real number."""
    if text in (None, ""):
        return None
    try:
...
def mlb_hand_lane(full_name: str) -> dict | None:
    """This season's MLB line BY BATTER HAND, or None.

    An arm with no PropFinder block used to drop to a single season BAA, which says
    nothing about who he is hard on -- and on a sheet whose whole premise is platoon
    lanes, one overall number is the least useful thing in the header. MLB carries
    the real vs-LHB and vs-RHB book for anyone who has thrown, so read that instead
    and keep the bare season figure as the last resort behind it.

    HR/9 is derived per hand from that hand's own home runs and innings, so it is
    measured rather than apportioned from the season total.
    """
    if full_name in _MLB_HAND_LANE_CACHE:
        return _MLB_HAND_LANE_CACHE[full_name]
    import urllib.parse
    import urllib.request
        ...
        if len(people) == 1:
            stats = json.load(
                urllib.request.urlopen(
                    ...
                    f"/stats?stats=statSplits&sitCodes=vl,vr&season={DATE[:4]}&group=pitching",
                    timeout=30,
                )
            )
            found: dict = {}
            for st in stats.get("stats", []):
                for sp in st.get("splits", []):
                    ...
                    if code in ("vl", "vr"):
                        found[code] = {
                            "avg": s.get("avg"),
                            "hr9": (float(hr) * 9.0 / ip) if (ip and hr is not None and ip > 0) else None,
                            "ip": ip,
                        }
            # Both hands or nothing: one hand alone reads like a platoon split when it
            # is really half a book, and half a book is how a lane gets misread.
            #
            # And each hand needs a real sample. Jared Simpson came in with four batters
            # faced from the left and ten from the right, no hits in either, so MLB
            # reports .000 twice -- accurate, and meaningless. Printed, it tells a bettor
            # lefties cannot touch a pitcher with a 6.75 ERA. PropFinder had already
            # written "-" for exactly that reason; this fallback was filling in the rate
            # the data provider had deliberately withheld. Same floor as a derived HR/9.
            if (
                found.get("vl")
                and found.get("vr")
                and (found["vl"].get("ip") or 0) >= MIN_IP_FOR_DERIVED_HR9
                and (found["vr"].get("ip") or 0) >= MIN_IP_FOR_DERIVED_HR9
            ):
                out = found
    except Exception:
        out = None
    _MLB_HAND_LANE_CACHE[full_name] = out
    return out


_THIN_BOOK_CACHE: dict[str, str | None] = {}


def mlb_thin_book(full_name: str) -> str | None:
    """What IS true about an arm whose book is too small to split: how small it is.

    A first career start with four innings behind it is a real tail angle -- the
    same one the "MLB debut, no book" line already names for zero innings -- and it
    is far more honest than a rate. Returns e.g. "first MLB start, 4.0 career IP", or
    None when the arm has a book big enough to be read on its own terms.
    """
    if full_name in _THIN_BOOK_CACHE:
        return _THIN_BOOK_CACHE[full_name]
    import urllib.parse
    import urllib.request

    label = None
    try:
        pid = load_probable_ids(DATE).get(full_name)
        if pid is None:
            url = "https://statsapi.mlb.com/api/v1/people/search?names=" + urllib.parse.quote(full_name)
            people = [
                p
        ...
        if pid is not None:
            ...
            for st in stats.get("stats", []):
                for sp in st.get("splits", []):
                    s = sp.get("stat") or {}
                    ip = _ip_to_float(s.get("inningsPitched"))
                    if ip is not None and 0 < ip < MIN_IP_FOR_DERIVED_HR9:
                        shown = str(s.get("inningsPitched"))
                        label = (
                            f"first MLB start, {shown} career IP"
                            if not s.get("gamesStarted")
                            else f"{shown} career IP"
                        )
    except Exception:
        label = None
    _THIN_BOOK_CACHE[full_name] = label
    return label
...
def mlb_season_lane(full_name: str) -> str | None:
    ...
    try:
        ...
        if len(people) == 1:
            ...
            for st in stats.get("stats", []):
                for sp in st.get("splits", []):
                    # a season average over a handful of at-bats is not a lane either
                    ip = _ip_to_float(sp["stat"].get("inningsPitched"))
                    if ip is not None and ip >= MIN_IP_FOR_DERIVED_HR9:
...
def fade_reason(
    ...
    if park_ctx:
        # The weather and stadium halves are only reportable when they were actually
        # measured. Without them the net park number still stands on its own; it just
        # cannot be attributed to wind or to the yard.
        _wx = park_ctx.get("hr_weather")
        _st = park_ctx.get("hr_stadium")
        if park_ctx["hr_pct"] <= -5:
            parts.append(f"park/weather net drag ({park_ctx['hr_pct']:+d}%)")
        elif _wx is not None and _wx <= -4:
            parts.append(f"weather carry headwind ({_wx:+d}%)")
        elif _st is not None and _st <= -6:
            parts.append(f"park suppresses carry ({_st:+d}%)")
...
def strip_marks(prop: str) -> str:
    """Prop text minus the favourite/gem markers."""
    return prop.replace("⭐", "").replace("💎", "").strip()


def read_matchup_teams(path: Path) -> tuple[str, str]:
    """(game key, opposing team) straight off a matchup export's header."""
    game = opp = ""
    with path.open(encoding="utf-8-sig", newline="") as f:
        for row in csv.reader(f):
            if not row:
                continue
            if row[0] == "Matchup" and len(row) > 1:
                game = row[1].strip()
            elif row[0] == "Opposing Team" and len(row) > 1:
                opp = row[1].strip()
            elif row[0] == "SPLIT":
                break
    return game, opp
...
def posted_lineup_names(game: str, team: str) -> list[str]:
    """Today's posted batting order for one team, from the MLB feed.

    Empty when the lineup is not out yet, which the caller treats as unknown
    rather than as nobody -- an unknown lineup cannot clear a broken export.
    """
    path = ROOT / "preview" / "data" / f"research-{DATE}.json"
    if not path.is_file():
        return []
    ...
    for g in data.get("games", []):
        ...
        if key != game:
            continue
        field = "awayLineup" if g.get("away") == team else "homeLineup"
        return [
            p.get("name", "")
            for p in (g.get(field) or [])
            if p.get("order") and p["order"] <= 9 and p.get("name")
        ]
    return []
...
def main() -> int:
    ...
    # A PropFinder export saved mid-load contains the literal "Loading matchup..."
    # and no batter rows. It parses fine and produces a game with an empty side --
    # once that silently removed every Detroit bat from the board.
    #
    # What makes that dangerous is a LOST PICK, not an empty file: a prop the sheet
    # was asked to carry quietly vanishing. So the check asks that question directly.
    # It reads today's posted lineup for the side the broken file covers, and if any
    # requested prop is in it, the build stops -- that batter cannot be scored from
    # yesterday's numbers and must not be dropped without a word either. If nobody on
    # the list bats on that side, the file costs the board nothing and the run
    # continues with the gap named out loud.
    _empty = []
    for _path in manifest_matchup_files(DATE):
        _rows = read_batter_rows(_path)
        _real = [r for r in _rows if r.get("name") and "Loading" not in r["name"]]
        if len(_real) < 5:
            _empty.append((_path, len(_real)))
    if _empty:
        _wanted = {name_lookup_key(strip_marks(p)) for p in RAW_PROPS}
        for _path, _n in _empty:
            _game, _opp = read_matchup_teams(_path)
            _posted = posted_lineup_names(_game, _opp)
            _lost = sorted(n for n in _posted if name_lookup_key(n) in _wanted)
            if _lost or not _posted:
                raise SystemExit(
                    f"{_path.name} has {_n} usable batter rows (export captured mid-load?) — "
                    + (
                        "re-export it: the sheet is carrying "
                        + ", ".join(_lost)
                        + " from that side and they cannot be scored without it"
                        if _lost
                        else "re-export it: no posted lineup to check it against, so "
                        "there is no way to tell what the board is losing"
                    )
                )
            print(
                f"  NOTE {_path.name} exported empty — no {_opp} bat is on the prop list, "
                f"so {_game} keeps both arms and loses no pick"
            )
    ...
    # The sheet renders each row under the spelling the prop list used, which also
    # ships the prop list's typos: 2026-09-22 arrived with "Alejandro kirk". When the
    # export holds the same name under different capitalisation, take the export's --
    # it is the same string, correctly cased, so nothing is invented.
    export_case = {name_lookup_key(e["row"]["name"]): e["row"]["name"] for e in batter_ctx.values()}

    def canonical_case(name: str) -> str:
        """Render a prop under the export's spelling when the two differ trivially.

        The prop list is typed by hand and its typos used to ship: 2026-09-05 sent
        "Alejandro kirk", 2026-09-22 sent "Alejanro Kirk", which matched nothing at
        all and dropped the prop off the sheet. Accept a difference of case, or a
        single character, but only when exactly one export name is that close --
        anything less certain stays missing and is reported.
        """
        key = name_lookup_key(name)
        exported = export_case.get(key)
        if exported and exported != name and exported.lower() == name.lower():
            print(f"  prop spelling: {name!r} -> {exported!r} (export capitalisation)")
            return exported
        if exported:
            return name
        near = [
            v
            for k, v in export_case.items()
            if abs(len(k) - len(key)) <= 1 and _within_one_edit(k, key)
        ]
        if len(set(near)) == 1:
            print(f"  prop spelling: {name!r} -> {near[0]!r} (one character off the export)")
            return near[0]
        return name
    ...
    for raw in RAW_PROPS:
        ...
        prop_entries.append((canonical_case(clean), fav, gem, game_override))
    ...
    for name, is_fav, is_gem, game_override in prop_entries:
        ...
        opp_throws = opposing_sp_throws(games_csv[ctx["game"]], ctx["opp_sp"])
        split = batter_split(hand, sp_risk, opp_throws)
    ...
    for gm in game_meta:
        ...
        if park:
            # The stadium/weather split is a separate measurement from the park
            # number itself. Today's source carries the total and not the split, so
            # the sentence stops at the total rather than claiming a 0/0 breakdown.
            park_line = f"Park boost {park['hr_pct']:+d}%"
            if park.get("hr_stadium") is not None and park.get("hr_weather") is not None:
                park_line += (
                    f" (stadium {park['hr_stadium']:+d}%, "
                    f"weather {park['hr_weather']:+d}%)"
                )
            park_line += "."
        else:
        ...
        def _sp_line(label: str, r: dict | None, side: str, sp_full: str = "") -> str:
            ...
            thin = mlb_thin_book(sp_full) if sp_full else None
            if thin:
                return f"{label} - thin book: {thin}"
            ...
            # MLB's own by-hand book, when he has one. Same shape as every other
            # header on the board -- BAA per hand plus HR/9 derived from that hand's
            # own homers and innings -- so a replaced probable reads like a starter
            # rather than like a hole.
            hand = mlb_hand_lane(sp_full) if sp_full else None
            if hand:
                # Written in the shape the header parser already reads, so this lane
                # renders like every other arm's instead of needing its own branch.
                bits = []
                if hand["vl"].get("avg"):
                    bits.append(f"BAA vs LHB {hand['vl']['avg']}")
                if hand["vr"].get("avg"):
                    bits.append(f"vs RHB {hand['vr']['avg']}")
                hr9s = [hand[c]["hr9"] for c in ("vl", "vr")]
                if all(v is not None for v in hr9s):
                    bits.append(f"HR/9 {hr9s[0]:.2f} vs LHB, {hr9s[1]:.2f} vs RHB")
                if bits:
                    return f"{label} ({', '.join(bits)})"
        ...
        if k_lines:
            # repr, not json.dumps: this is written into a PYTHON module, and a None
            # serialised as JSON null makes the built sheet fail to import.
            lines.append(f'        "kLines": {k_lines!r},')
        ...
        for p in prop_by_game[gm["key"]]:
            ...
            if risk and risk.get("no_data"):
                ...
                chip_throws = opposing_sp_throws(gm, chip)
                split = batter_split(hand, risk, chip_throws)
                if hand == "S":
                    # opposite the arm, never the friendlier number
                    split_side = switch_side(chip_throws)
                    matchup = (
            ...
            _bk = name_lookup_key(name)
            _bp = batter_profiles.get(_bk)
            if _bp is None:
                _parts = _bk.split()
                if len(_parts) >= 2:
                    _base = (
    ...
    print(
        ...
        # Count MARKED PROPS, not unique names. On a doubleheader the same hitter is
        # two separate bets -- Bellinger is a favorite in both games -- and the deduped
        # display list made the log read 13/20 against a board carrying 14/21, which
        # looks exactly like a dropped marker.
        f"{len(fav_keys)} favorites, {len(gem_keys)} hidden gems"
        + (
            f" ({len(favs_display)}/{len(gems_display)} unique names)"
            if (len(fav_keys), len(gem_keys)) != (len(favs_display), len(gems_display))
            else ""
        )
    )
    if missing:
