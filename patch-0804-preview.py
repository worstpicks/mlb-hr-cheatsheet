#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-04. Does not commit or push."""
...
SHEET_DATE = "2026-08-04"

spec = importlib.util.spec_from_file_location("build0804", ROOT / "build-sheet-2026-08-04.py")
...
# 8/4 judgment: O0.5 Harper (LHB +0.76 vs Littell 1.03 🧤, CBP +26, 2 HR / 2 near).
# O1.5 Caissie (2 HR / 3 near, LHB +0.52 vs Holmes) — different game.
_harper = next((r for r in straight_rows if r["name_plain"] == "Bryce Harper"), None)
_caissie = next((r for r in straight_rows if r["name_plain"] == "Owen Caissie"), None)
if _harper is not None:
    straight_o05 = _harper
...
# 8/4 Goblin judgment: loudest attack lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Shohei Ohtani", "Carter Jensen", "Nelson Velazquez"]
_two_lock_names = ["Salvador Perez", "Dylan Crews"]
_fav3_lock_names = ["Pete Alonso", "Junior Caminero", "Griffin Conine"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 4, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-03",
        "2026-08-02",
