#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-08-22.

Two arms, both replaced after the export was pulled:

  STL  Matthew Liberatore -> Quinn Mathews   MLB still lists St. Louis as TBD; the
                              sheet's owner named Mathews. Batter rows come from the
                              Liberatore export -- same Philadelphia lineup, and
                              batter form does not depend on who is pitching -- and
                              his Season/vsLHB/vsRHB block is carried from his last
                              real export (2026-08-17).

  SEA  Bryan Woo -> Kade Anderson            Anderson is a debut arm (0-0, 0.00). He
                              takes the Woo export's Chicago batter rows; no pitcher
                              block exists for him anywhere, so his header carries no
                              invented split.
"""
...
GAPS: dict[str, dict] = {
    "Quinn Mathews": {
        ...
        "oteam": "PHI",
        "lineups": "sibling",
        "sibling": f"hr-matchups-STL-at-PHI-Matthew-Liberatore-{DATE}.csv",
        "block_from": "hr-matchups-STL-at-CIN-Quinn-Mathews-2026-08-17.csv",
    },
    "Kade Anderson": {
        "matchup": "CHC @ SEA",
        "pteam": "SEA",
        "oteam": "CHC",
        "lineups": "sibling",
        "sibling": f"hr-matchups-CHC-at-SEA-Bryan-Woo-{DATE}.csv",
        "block_from": None,
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-STL-at-PHI-Matthew-Liberatore-{DATE}.csv",
    f"hr-matchups-CHC-at-SEA-Bryan-Woo-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 8/22 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# Nothing is carried. Neither replacement arm has a PropFinder risk row, and there is
# no prior score for either to carry forward -- Mathews last threw on 8/17 without one
# and Anderson is a debut. Inventing a number would put it on a different scale from
# every other arm on the board.
