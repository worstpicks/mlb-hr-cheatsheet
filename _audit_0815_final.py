#!/usr/bin/env python3
"""Final local audit for 2026-08-15 (do not push)."""
...
def main() -> int:
    ...
    if "Saturday, August 15, 2026" not in html:
        fail("hero must be Saturday, August 15, 2026")
    if "Friday, August 14, 2026 — Worst" in html:
        fail("stale 8/14 hero still on current sheet")
    if "Friday, August 15" in html or "Sunday, August 15" in html:
        fail("wrong weekday on August 15 (must be Saturday)")
    if "August 15, 2026 — current slate" not in html:
        fail("manifest missing August 15 current slate")
    if "August 14, 2026 — current slate" in html:
        fail("8/14 still labeled current")
    ...
    expected_bum = {"Justin Wrobleski", "Michael Lorenzen", "Jared Jones"}
    ...
    arch = ROOT / "preview" / "archive" / "2026-08-14.html"
    if not arch.exists():
        fail("8/14 archive missing")
    elif 'content="2026-08-14"' not in arch.read_text(encoding="utf-8"):
        fail("8/14 archive wrong sheet-date")
