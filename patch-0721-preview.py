#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-21. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-20.html"
SHEET_DATE = "2026-07-21"

spec = importlib.util.spec_from_file_location("build0721", ROOT / "build-sheet-2026-07-21.py")
...
# 7/21 client pick: Drake Baldwin O0.5 vs Buehler 🧤 (SD @ ATL); keep Mayo O1.5.
_o05_lock = rows_by_plain_early.get("Drake Baldwin")
if (
    _o05_lock
    and straight_o15
    and _o05_lock["game_key"] != straight_o15["game_key"]
    and _o05_lock in straight_o05_pool(straight_rows, strict=True)
):
...
def update_manifest():
    ...
    if "2026-07-20" in old or (ARCHIVE_PREVIOUS.is_file()):
        old["2026-07-20"] = {
            "date": "2026-07-20",
            "label": "July 20, 2026",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "July 21, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-07-20",
        "2026-07-19",
        ...
        elif date == "2026-07-19":
            ordered.append(
                {
                    "date": "2026-07-19",
                    "label": "July 19, 2026",
                    "href": "archive/2026-07-19.html",
                }
            )
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Tuesday, July 21, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
