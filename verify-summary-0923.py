...
# Inspect the patch, do not re-run its side effects: without this flag the exec below
# retook the lineup snapshot after the page had been built from the earlier one.
import os

os.environ["WORSTPICKZ_AUDIT_READONLY"] = "1"
patch_globals: dict = {"__file__": str(ROOT / "patch-0923-preview.py"), "__name__": "__main__"}
cut = (ROOT / "patch-0923-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
exec(compile(cut + "\n", "patch-0923-preview.py", "exec"), patch_globals)
...
# Expect what a picker OBEYING THE RULES could fill, not every bat that passes the
# weather gate. On 9/16 eight bats passed it, but one was benched and three were a
# third, fourth and fifth Coors bat behind a two-per-game cap -- so the card correctly
# held four and this check failed a correct sheet. Count starting bats, game caps
# applied, and a shortfall below THAT is a real picker miss worth failing on.
_caps = patch_globals.get("weather_game_caps") or {}
_base_cap = patch_globals.get("weather_max_per_game", 2)
_per_game: Counter = Counter()
_achievable = 0
for _r in sorted(weather_eligible, key=lambda r: -r["score"]):
    if not _r.get("is_starting", True):
        continue
    if _per_game[_r["game_key"]] >= _caps.get(_r["game_key"], _base_cap):
        continue
    _per_game[_r["game_key"]] += 1
    _achievable += 1
weather_expected = min(5, _achievable)
...
# The heading counts what the card actually holds, so it is not a fixed string --
# splitting on the literal "Top 5 ..." crashed the moment the parks supported only
# two. Anchor on the stable part of the heading instead.
_wh = re.search(r"Top \d+ Weather Heavy HR Play", text)
if not _wh:
    raise SystemExit("verify-summary: Weather Heavy card not found in the rendered sheet")
wh_block = text[_wh.end():].split("Best longshot")[0]
if "split -" in wh_block:
    errors.append("Weather Heavy HTML contains negative split play")

if errors:
    print("FAIL verify-summary-0923:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK verify-summary-0923")
