#!/usr/bin/env python3
"""Final local audit for 2026-08-12 (do not push)."""
...
def main() -> int:
    ...
    if "Wednesday, August 12, 2026" not in html:
        fail("hero must be Wednesday, August 12, 2026")
    if "Tuesday, August 11, 2026 — Worst" in html:
        fail("stale 8/11 hero still on current sheet")
    if "Tuesday, August 12" in html or "Thursday, August 12" in html:
        fail("wrong weekday on August 12 (must be Wednesday)")
    if "August 12, 2026 — current slate" not in html:
        fail("manifest missing August 12 current slate")
    if "August 11, 2026 — current slate" in html:
        fail("8/11 still labeled current")
    ...
    arch = ROOT / "preview" / "archive" / "2026-08-11.html"
    if not arch.exists():
        fail("8/11 archive missing")
    elif 'content="2026-08-11"' not in arch.read_text(encoding="utf-8"):
        fail("8/11 archive wrong sheet-date")
