#!/usr/bin/env python3
"""Fill hr-matchups PropFinder did not export for 2026-09-23.

  Seven arms shipped no matchup file. Four have today's zone lineups, HR risk and
  pitcher summaries, so they are rebuilt from those plus each hitter's latest real
  batted-ball form: Abbott and Sale (CIN @ ATL), Kirby (HOU @ SEA), Lugo (CWS @ KC).

  Fedde is the owner's PRIM arm for Chicago; PropFinder built the game around Bryan
  Hudson, an opener (8 starts, 4.2 batters). Kansas City's lineup comes from the
  zone rows filed under Hudson, but ZONE is blanked: Hudson throws left, Fedde right.

  TOR @ BAL game two (Van Eyk / Gibson) has no export of any kind. Both lineups are
  the same nine as game one, so the batter rows are copied from game one's real
  export with ODDS and ZONE blanked -- form carries across a doubleheader, zone fit
  and prices do not.
"""
...
GAPS: dict[str, dict] = {
    "Andrew Abbott": {"matchup": "CIN @ ATL", "pteam": "CIN", "oteam": "ATL", "lineups": "zone"},
    "Chris Sale": {"matchup": "CIN @ ATL", "pteam": "ATL", "oteam": "CIN", "lineups": "zone"},
    "George Kirby": {"matchup": "HOU @ SEA", "pteam": "SEA", "oteam": "HOU", "lineups": "zone"},
    "Seth Lugo": {"matchup": "CWS @ KC", "pteam": "KC", "oteam": "CWS", "lineups": "zone"},
    "Erick Fedde": {
        "matchup": "CWS @ KC", "pteam": "CWS", "oteam": "KC", "lineups": "zone",
        "zone_pitcher": "Bryan Hudson", "drop_zone": True,
    },
    "CJ Van Eyk": {
        "matchup": "TOR @ BAL", "pteam": "TOR", "oteam": "BAL", "lineups": "sibling",
        "sibling": f"hr-matchups-TOR-at-BAL-Max-Scherzer-{DATE}.csv", "drop_zone": True,
    },
    "Trey Gibson": {
        "matchup": "TOR @ BAL", "pteam": "BAL", "oteam": "TOR", "lineups": "sibling",
        "sibling": f"hr-matchups-TOR-at-BAL-Chris-Bassitt-{DATE}.csv", "drop_zone": True,
    },
}
...
def load_zone_lineups() -> dict[str, list[dict]]:
    ...
    # a gap arm may need the lineup that zone-matchups filed under a different
    # pitcher (a doubleheader's other game, or a probable changed after export)
    want = {}
    for name, spec in GAPS.items():
        want[spec.get("zone_pitcher", name)] = name
    with (DATA / f"zone-matchups-{DATE}.csv").open(encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            target = want.get(row["Pitcher"])
            if target:
...
def pitcher_block_rows(pitcher: str, summaries: dict[str, dict[str, dict]]) -> list[list[str]]:
    spec = GAPS[pitcher]
    # An export can lose its batter table and keep its pitcher block. When it does,
    # that block is today's real one and beats anything rebuilt from the summaries,
    # so block_from is honored whatever the lineup source is.
...
def batter_row(zrow: dict, prior_index: dict[str, dict], drop_zone: bool = False) -> tuple[list[str], bool]:
    ...
    zone = "" if drop_zone else (zrow.get("ZoneScore") or "")
    if zone:
        try:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 9/4 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
def weakspots_lineup(pitcher: str, matchup: str) -> list[dict]:
    """Today's posted order against this arm, read off its weak-spots export.

    The zone feed is the usual source, but it skipped this arm entirely. The
    weak-spots export for the same pitcher did not: it carries the nine hitters,
    their handedness and their slot, all measured today. There is no ZoneScore in
    it, so ZONE stays blank rather than borrowed -- zone fit is specific to the arm
    on the mound and there is no honest substitute for it.
    """
    path = DATA / f"pitcher-weak-spots-{slug(matchup.replace(' @ ', '-at-'))}-{slug(pitcher)}-{DATE}.csv"
    if not path.is_file():
        print(f"  WARN weak-spots source missing: {path.name}")
        return []
    out: list[dict] = []
    seen: set[str] = set()
    for line in path.read_text(encoding="utf-8-sig").splitlines():
        ...
        if not row or not row[0].strip():
            continue
        m = re.match(r"^(.*?)\s+(LHB|RHB|SHB)$", row[0].strip())
        if not m:
            continue
        name, hand = m.group(1).strip(), m.group(2)
        if name in seen:
            continue
        seen.add(name)
        out.append({"Batter": name, "Bats": hand, "ZoneScore": ""})
    return out


def sibling_batter_rows(sibling: str, drop_zone: bool = False) -> tuple[list[list[str]], list[str]]:
    ...
    # ZONE was measured against the sibling game's pitcher. Form carries across a
    # doubleheader; zone fit does not, so a different arm gets it blanked.
    zone_i = header.index("ZONE") if (drop_zone and "ZONE" in header) else None
    out: list[list[str]] = []
    for line in lines[hdr_i + 1 :]:
        row = next(csv.reader([line]), None)
        if not row or not row[0].strip():
...
def write_game(pitcher: str, zrows, summaries, prior_index) -> str | None:
    ...
    if is_real_export(dst) and not spec.get("force"):
        # a file that exists but carries no batters is not a real export
        try:
            body = dst.read_text(encoding="utf-8-sig", errors="ignore")
            usable = sum(body.count(tag) for tag in ("HB,", "HB" + chr(10)))
        except OSError:
            usable = 0
        if usable >= 5:
            print(f"skip {dst.name} - real PropFinder export present")
            return dst.name
        print(f"rebuilding {dst.name} - export present but has no batter rows")

    if spec["lineups"] == "weakspots":
        zrows = weakspots_lineup(pitcher, matchup)
    if spec["lineups"] == "sibling":
        rows, header = sibling_batter_rows(spec["sibling"], spec.get("drop_zone", False))
        ...
        with_form = 0
        for z in zrows:
            row, had = batter_row(z, prior_index, spec.get('drop_zone', False))
...
# Nothing is carried. Williams has no same-day HR-risk row -- PropFinder built the
# export around Rocker -- and Rocker's row is Rocker's, which is the point of not
# using it. Williams has a real 2026 book, so his header renders measured rates.
