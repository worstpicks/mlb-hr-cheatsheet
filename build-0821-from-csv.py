#!/usr/bin/env python3
"""Build 2026-08-21 sheet from imported CSVs and user prop list."""
...
DATE = "2026-08-21"
OUT = ROOT / "build-sheet-2026-08-21.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0821-matchups.py. Nothing is carried today: every arm the sheet
...
# Two Max Muncys on this slate: Oakland's DH (R, 9th) and the Dodgers' 3B (L, 4th).
# Without an entry here the prop resolves by dict-iteration order -- which is
# alphabetical by game key, i.e. arbitrary. The user's list groups props game by
# game, and this one sits between Michael Conforto (CHC) and Hunter Feduccia (LAD),
# inside the PIT @ LAD block. The listed hand (L) agrees.
BATTER_GAME_OVERRIDES: dict[str, str] = {"Max Muncy": "PIT @ LAD"}
...
def main() -> int:
    ...
    for gm in game_meta:
        ...
        def _sp_line(label: str, r: dict | None, side: str, sp_full: str = "") -> str:
            ...
            # Nothing measured anywhere -> name the arm and stop.
            return label
