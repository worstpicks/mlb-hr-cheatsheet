#!/usr/bin/env python3
"""Build 2026-06-24 sheet from imported CSVs and user prop list."""
...
DATE = "2026-06-24"
OUT = ROOT / "build-sheet-2026-06-24.py"
BUM_RISK_MIN = 0.95
...
RAW_PROPS = [
    ...
    "Juan Soto(Game 1)⭐",
    "MJ Melendez(Game 1)",
    "Francisco Alvarez(Game 1)",
    "Eric Wagaman💎(Game 1)",
    "Carson Benge(Game 1)",
    "Pete Crow Armstrong(Game 1)⭐",
    "Ian Happ(Game 1)",
    "Colson Montgomery",
    "Miguel Vargas",
    "Randal Grichuk",
    ...
    "Carson Benge(Game 2)",
    "Bo Bichette (Game 2)",
    "Francisco Alvarez(Game 2)",
    "Seiya Suzuki⭐(Game 2)",
    "Dansby Swanson(Game 2)",
    "Carson Kelly(Game 2)",
    "Ian Happ(Game 2)",
    "Tyler Stephenson",
    "JJ Bleday",
...
CHC_DH_SPECS = {
    "CHC @ NYM (G1)": {
        "away_sp": "Imanaga",
        "home_sp": "Manaea",
        "away_sp_full": "Shota Imanaga",
        "home_sp_full": "Sean Manaea",
        "files": [
            "hr-matchups-CHC-at-NYM-Shota-Imanaga-2026-06-24.csv",
            "hr-matchups-CHC-at-NYM-Sean-Manaea-2026-06-24.csv",
        ],
    },
    "CHC @ NYM (G2)": {
        "away_sp": "Assad",
        "home_sp": "McLean",
        "away_sp_full": "Javier Assad",
        "home_sp_full": "Nolan McLean",
        "files": [
            "hr-matchups-CHC-at-NYM-Javier-Assad-2026-06-24.csv",
            "hr-matchups-CHC-at-NYM-Nolan-McLean-2026-06-24.csv",
        ],
    },
}
...
def split_chc_doubleheader(games: dict[str, dict]) -> None:
    if "CHC @ NYM" not in games:
        return
    games.pop("CHC @ NYM", None)
...
def parse_prop_label(raw: str) -> tuple[str, bool, bool, str | None]:
    ...
    if m:
        clean = "Max Muncy"
        game_override = "LAD @ MIN" if m.group(1).upper() == "LAD" else "ATH @ SF"
