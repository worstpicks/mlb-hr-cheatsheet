...
patch_globals: dict = {"__file__": str(ROOT / "patch-0623-preview.py"), "__name__": "__main__"}
cut = (ROOT / "patch-0623-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
exec(compile(cut + "\n", "patch-0623-preview.py", "exec"), patch_globals)
...
for label, picked, ok_fn in (
    ...
    expected = weather_expected if label == "Top 5 Weather Heavy HR Plays" else 5
    ...
    weather_cap = patch_globals.get("weather_max_per_game", 2) if label == "Top 5 Weather Heavy HR Plays" else 2
...
if len(longshots) < 1:
    errors.append(f"Longshots: expected at least 1, got {len(longshots)}")
if len(longshots) > 4:
...
if errors:
    print("FAIL verify-summary-0623:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK verify-summary-0623")
