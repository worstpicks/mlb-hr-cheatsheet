...
patch_globals: dict = {"__file__": str(ROOT / "patch-0818-preview.py"), "__name__": "__main__"}
cut = (ROOT / "patch-0818-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
exec(compile(cut + "\n", "patch-0818-preview.py", "exec"), patch_globals)
...
for label, picked, ok_fn in (
    ...
    if label == "Top 5 Weather Heavy HR Plays":
        ...
        # flat number failed a correct sheet on 2026-08-18, when Milwaukee's +22%
...
# 2026-08-18: the rubric moved to 4 per game (a parlay is not a portfolio -- see
...
if errors:
    print("FAIL verify-summary-0818:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK verify-summary-0818")
