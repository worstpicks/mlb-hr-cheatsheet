#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-01. Does not commit or push."""
...
SHEET_DATE = "2026-08-01"

spec = importlib.util.spec_from_file_location("build0801", ROOT / "build-sheet-2026-08-01.py")
...
# 8/1 judgment: O0.5 Seager (top zone/split vs Blanco) · O1.5 Mayo (4 HR / 5 near multi-HR).
_seager = next((r for r in straight_rows if r["name_plain"] == "Corey Seager"), None)
_mayo = next((r for r in straight_rows if r["name_plain"] == "Coby Mayo"), None)
if _seager is not None:
    straight_o05 = _seager
...
def update_manifest():
    ...
    if "2026-07-31" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-07-31"] = {
            "date": "2026-07-31",
            "label": "July 31, 2026",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 1, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-07-31",
        "2026-07-30",
