#!/usr/bin/env python3
"""Build 2026-08-19 sheet from imported CSVs and user prop list."""
...
DATE = "2026-08-19"
OUT = ROOT / "build-sheet-2026-08-19.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0818-matchups.py, because PropFinder built every 8/18 export
# around the superseded probable. Labelled on the row so a reader never mistakes a
# stale split for a same-day number.
CARRIED_RISK = {"Basso": "7/25", "Lynch IV": "8/12"}
...
ALIASES = {
    ...
    "Pete Crow-Armstrong": "Pete Crow-Armstrong",
    # The request lowercases the suffix; PropFinder exports "Ronald Acuna Jr.".
    ...
    # Seattle's shortstop is Rodden, spelled Rodeen in the request.
    "Brock Rodeen": "Brock Rodden",
    "Brock Rodden": "Brock Rodden",
...
def split_stl_cin_doubleheader(games: dict[str, dict]) -> None:
    # No specs means no doubleheader today. Without this guard the function popped
    # the single STL @ CIN game and put nothing back, silently dropping a whole game
    # and its eight listed props.
    if not STL_CIN_DH_SPECS:
        return
...
def parse_prop_label(raw: str) -> tuple[str, bool, bool, str | None]:
    ...
    if m:
        clean = "Max Muncy"
        game_override = "LAD @ COL" if m.group(1).upper() == "LAD" else "ATH @ KC"
...
def main() -> int:
    ...
    for gm in game_meta:
        ...
        def _sp_line(label: str, r: dict | None, side: str) -> str:
            # Nothing measured for this arm -> name it and stop. A "no HR data"
            # placeholder is noise on a betting sheet; the reader gains nothing.
            if not r or r.get("no_data"):
                return label
            return (
                f"{label} "
