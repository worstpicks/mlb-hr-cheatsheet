#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-08. Does not commit or push."""
...
SHEET_DATE = "2026-08-08"

spec = importlib.util.spec_from_file_location("build0808", ROOT / "build-sheet-2026-08-08.py")
...
# 8/8 judgment: O0.5 Hill vs Scherzer 🧤 (+1.05 / 1.49, CBP +39, 2 HR / 2 near).
# O1.5 Contreras vs Jump (2 HR / 2 near) — different game.
_hill = next((r for r in straight_rows if r["name_plain"] == "Derek Hill"), None)
_contreras = next((r for r in straight_rows if r["name_plain"] == "Willson Contreras"), None)
if _hill is not None:
    straight_o05 = _hill
if _contreras is not None and _contreras["game_key"] != straight_o05["game_key"]:
    straight_o15 = _contreras
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/8 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Jesus Sanchez", "Ronald Acuna Jr.", "Jordan Walker"]
# Mesa over Endy: stronger zone-fit / attack after straights + 3-leg (+0.47 / 0.58, 3 near-HR).
_two_lock_names = ["Victor Mesa Jr.", "Mike Trout"]
_fav3_lock_names = ["Brandon Lowe", "Junior Caminero", "Munetaka Murakami"]
...
def update_manifest():
    ...
    if "2026-08-04" in old or (ROOT / "preview" / "archive" / "2026-08-04.html").is_file():
        old["2026-08-04"] = {
            "date": "2026-08-04",
            "label": "August 4, 2026",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 8, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-07",
        "2026-08-06",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Saturday, August 8, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
