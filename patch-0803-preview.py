#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-03. Does not commit or push."""
...
SHEET_DATE = "2026-08-03"

spec = importlib.util.spec_from_file_location("build0803", ROOT / "build-sheet-2026-08-03.py")
...
# 8/3 judgment: O0.5 Abrams (LHB +1.28 vs Nola 1.45 🧤) · O1.5 Pena (2 HR / 3 near, RHB +1.41 vs Bieber).
_abrams = next((r for r in straight_rows if r["name_plain"] == "CJ Abrams"), None)
_pena = next((r for r in straight_rows if r["name_plain"] == "Jeremy Pena"), None)
if _abrams is not None:
    straight_o05 = _abrams
if _pena is not None and _pena["game_key"] != straight_o05["game_key"]:
    straight_o15 = _pena
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/3 Goblin judgment: loudest attack lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Victor Mesa Jr.", "Esmerlyn Valdez", "Ben Rice"]
_two_lock_names = ["Dylan Crews", "Cole Carrigg"]
_fav3_lock_names = ["George Springer", "Spencer Jones", "Spencer Horwitz"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 3, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-02",
        "2026-08-01",
