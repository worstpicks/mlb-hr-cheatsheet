#!/usr/bin/env python3
"""Hard verify June 9 sheet: duplicates, counts, summary, straights, bums."""
...
if sheet_date != "2026-06-09":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Tuesday, June 9, 2026" not in text:
    errors.append("header weekday wrong: expected Tuesday, June 9, 2026")
if "Friday, June 9, 2026" in text:
    errors.append("header still says Friday, June 9, 2026")
...
stale_june8 = ["Jackson Chourio", "Eric Haase", "Jonathan Aranda", "Jeffrey Springs", "Grayson Rodriguez"]
for stale in stale_june8:
    if stale in text:
        errors.append(f"stale June 8 name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0609:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0609")
