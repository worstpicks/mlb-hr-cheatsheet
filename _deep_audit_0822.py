#!/usr/bin/env python3
"""Deep content audit for the 2026-08-22 sheet: prop coverage, markers, bums, staleness."""
...
EXPECTED_BUMS = {"Ryan Johnson", "Jake Irvin", "Jared Jones", "Luis Castillo"}

# Every game carries two pitcher segments. Quinn Mathews has no PropFinder HR-risk
# row -- he was named after the export -- so his header renders the real measured BAA
# lane carried from his 8/17 export instead of a risk score.
...
SUPERSEDED_PROBABLES: list[str] = ["Matthew Liberatore", "Bryan Woo"]
...
def main() -> int:
    ...
    for name in STALE_PREV_DAY:
        if fold(name) in body_folded:
            fail(f"stale 8/21 content on sheet: {name}")
