#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-05. Does not commit or push."""
...
SHEET_DATE = "2026-08-05"

spec = importlib.util.spec_from_file_location("build0805", ROOT / "build-sheet-2026-08-05.py")
...
# 8/5 judgment: O0.5 Lile (LHB +1.27 vs Painter 1.19 🧤, CBP +34, 2 HR / 2 near).
# O1.5 Mayo (3 HR / 4 near vs Detmers) — different game.
_lile = next((r for r in straight_rows if r["name_plain"] == "Daylen Lile"), None)
_mayo = next((r for r in straight_rows if r["name_plain"] == "Coby Mayo"), None)
if _lile is not None:
    straight_o05 = _lile
if _mayo is not None and _mayo["game_key"] != straight_o05["game_key"]:
    straight_o15 = _mayo
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/5 Goblin judgment: loudest attack lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Taylor Trammell", "Seiya Suzuki", "Junior Caminero"]
_two_lock_names = ["Owen Caissie", "Willi Castro"]
_fav3_lock_names = ["Bryce Harper", "Victor Mesa Jr.", "Alec Burleson"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 5, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-04",
        "2026-08-03",
