#!/usr/bin/env python3
"""Hard verify June 16 sheet: duplicates, counts, summary, straights, bums."""
...
if sheet_date != "2026-06-16":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Tuesday, June 16, 2026" not in text:
    errors.append("header weekday wrong: expected Tuesday, June 16, 2026")
...
stale_june15_only = [
    "Zach Neto", "Ian Happ", "Ben Williamson", "Kyle Karros",
    "Jac Caglianone", "Maikel Garcia", "Dane Myers", "JJ Wetherholt",
    "Corey Seager", "Isaac Paredes", "Gleyber Torres", "Max Muncy",
    "Henry Bolte", "Tyler Callihan", "Gabriel Moreno", "Freddie Freeman",
    "Walbert Urena", "Ryne Nelson", "Eric Lauer", "Shota Imanaga",
    "Michael Lorenzen", "Tobias Myers", "Chase Burns", "Mitch Spence",
    "Andrew Alvarez", "Ryan Gusto", "Zack Wheeler", "Lucas Giolito",
    "Dustin May", "Mike Paredes", "MacKenzie Gore", "Troy Melton",
    "Kai-Wei Teng", "Jared Jones", "J.T. Ginn", "Nick Martinez",
    "Owen Caissie", "Joe Mack", "Jacob Young", "Salvador Perez",
    "Dillon Dingler",
]
for stale in stale_june15_only:
    if stale in text:
        errors.append(f"stale prior-slate name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0616:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0616")
