...
ROOT = Path(".")
spec = importlib.util.spec_from_file_location("b", "build-sheet-2026-08-13.py")
b = importlib.util.module_from_spec(spec)
spec.loader.exec_module(b)
risk = load_pitcher_risk(ROOT / "data/hr-targets-overall-2026-08-13.csv")
rows = []
for g in enrich_games_list(b.games, "2026-08-13"):
...
for r in o05[:15]:
    ...
    print(
        ...
        f"park={int(r.get('hand_park_pct') or r['park_pct']):+d} HR={r['hr']}/{r['near']} {r['game_key']}"
    )
...
for r in o15[:12]:
    ...
    print(
        ...
        f"split={r['split']:+.2f} risk={r['risk']:+.2f} park={int(r.get('hand_park_pct') or r['park_pct']):+d} "
...
for r in sorted([r for r in rows if r["fav"]], key=hr_rank_sort_key, reverse=True)[:15]:
    print(
        ...
        f"split={r['split']:+.2f} risk={r['risk']:+.2f} park={int(r.get('hand_park_pct') or r['park_pct']):+d} HR={r['hr']}/{r['near']}"
    )
