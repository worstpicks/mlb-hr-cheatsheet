#!/usr/bin/env python3
"""Synthesize the hr-matchups game PropFinder did not export for 2026-09-04.

  TEX  Kumar Rocker -> Trevor Williams   MLB has Williams; both the pasted block
                          and PropFinder's export still carry Rocker. Williams
                          inherits the Rocker file's Tampa Bay batter rows --
                          batter form does not depend on who is pitching -- and
                          carries his own pitcher block from 2026-08-03.

Detroit at Cleveland is a split doubleheader. Game one was already in progress
when the slate was pulled and PropFinder exported nothing for it, so only game
two (Sears / Griffin) is on the board.
"""
...
GAPS: dict[str, dict] = {
    # PropFinder's Foster Griffin export was captured mid-load: the file contains
    # the literal string "Loading matchup..." and no batter rows, so Detroit's
    # bats vanished from the board entirely. Rebuild the lineup from zone-matchups
    # and the form from prior exports (batter form is pitcher-independent).
    #
    # zone_score is deliberately dropped: the only zone rows PropFinder shipped for
    # Detroit are keyed to Logan Allen, who starts game ONE of the doubleheader.
    # Carrying his zone numbers into a game-two matchup would be a fabrication.
    "Foster Griffin": {
        "matchup": "DET @ CLE",
        "pteam": "CLE",
        "oteam": "DET",
        "lineups": "zone",
        # the zone export keys Detroit's bats to game one's arm, so read the
        # lineup from there while refusing to carry its matchup-specific score
        "zone_pitcher": "Logan Allen",
        "drop_zone": True,
    },
    "Trevor Williams": {
        "matchup": "TB @ TEX",
        ...
        "oteam": "TB",
        "lineups": "sibling",
        "sibling": f"hr-matchups-TB-at-TEX-Kumar-Rocker-{DATE}.csv",
        "block_from": "hr-matchups-WSH-at-PHI-Trevor-Williams-2026-08-03.csv",
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-TB-at-TEX-Kumar-Rocker-{DATE}.csv",
]
