#!/usr/bin/env python3
"""Build 2026-09-09 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-09"
OUT = ROOT / "build-sheet-2026-09-09.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0909-matchups.py. Nothing is carried today: every arm the sheet
...
ALIASES = {
    # Two characters off, which the one-character resolver rightly refuses to guess
    # at -- "McMan" could plausibly have been several people. He sits in the owner's
    # Arizona run and James McCann is the only close name in that game.
    "James McMan": "James McCann",
...
# Both Max Muncys are separate props today and the owner tagged the Oakland one
# "(ATH)", which parse_prop_label resolves on its own. The untagged entry sits in
# the Dodgers run of his list, so it is the LA one.
...
PROBABLE_OVERRIDES: dict[str, dict] = {
    # The owner exported Didier Fuentes for Atlanta because his pitch mix is close
    # to the projected starter's; the arm taking the mound is Reynaldo Lopez. Only
    # the NAME moves -- Lopez has no risk row and no export of his own, so his
    # header falls through to his real MLB season figure rather than printing
    # Fuentes's measured lanes under Lopez's name.
    "TB @ ATL": {
        "side": "home",
        "from": "Fuentes",
        "to": "Lopez",
...
def main() -> int:
    ...
    # today that silently removed every Detroit bat from the board. Refuse it.
    for _path in manifest_matchup_files(DATE):
        ...
        if len(_real) < 5:
            raise SystemExit(
                f"{_path.name} has {len(_real)} usable batter rows "
                f"(export captured mid-load?) — re-export it or add a gap-fill spec"
            )
    ...
    # ships the prop list's typos: 2026-09-09 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-09 sent "Alejanro Kirk", which matched nothing at
