#!/usr/bin/env python3
"""Patch preview sheet to 2026-09-23. Does not commit or push."""
...
SHEET_DATE = "2026-09-23"

# The slate being replaced is whatever preview/index.html currently holds -- NOT
# "yesterday". No sheet was built for 8/23, so a hardcoded yesterday would not match
# the preview's own sheet-date, the archive guard would skip, and the 8/22 sheet
# would be overwritten without ever being archived. Derive it.
_prev_m = re.search(
    r'<meta name="sheet-date" content="([^"]+)">',
    (ROOT / "preview" / "index.html").read_text(encoding="utf-8"),
)
_prev_date = _prev_m.group(1) if _prev_m else ""
# On a re-run the preview already holds SHEET_DATE, and archiving it under its own
# date would list today's slate as both "current" and an archive entry. Point the
# archive at a date that cannot match, so the existing guard simply skips.
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / f"{_prev_date or '0000-00-00'}.html"
if _prev_date == SHEET_DATE:
    ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "__already-archived__.html"

spec = importlib.util.spec_from_file_location("build0923", ROOT / "build-sheet-2026-09-23.py")
...
# How many rows COULD carry zone data: a batter only gets a zoneScore when
# zone-matchups shipped a row for him. A fixed TOTAL_ROWS - 20 floor fails a correct
# sheet whenever PropFinder omits a whole game from the zone export (today it shipped
# no Arizona or San Francisco rows at all) or when a doubleheader's second game is
# keyed to the first game's arms. Measure the real shortfall instead.
# How many rows COULD carry zone data. Ask the same matcher the enricher uses --
# an estimator with its own looser rule counted rows the enricher would never
# match and then failed a correct sheet for the difference.
def _zone_eligible_count() -> int:
    from zone_matchups import load_zone_lookup, lookup_zone_row

    lookup = load_zone_lookup(SHEET_DATE)
    if not lookup:
        return 0
    n = 0
    for g in build.games:
        for r in g["rows"]:
            ...
            hit = lookup_zone_row(plain, chip, lookup)
            # lookup_zone_row falls back to a row with no zone_score at all;
            # those can never render a zoneScore field, so they are not eligible.
            if hit and hit.get("zone_score") is not None:
                n += 1
    return n


ZONE_ELIGIBLE = _zone_eligible_count()
ZONE_ROW_MIN = max(0, ZONE_ELIGIBLE - 3)
ZONE_ROW_COUNT = GAMES_BLOCK.count("zoneScore:")
if ZONE_ELIGIBLE < TOTAL_ROWS:
    print(
        f"zone export covers {ZONE_ELIGIBLE}/{TOTAL_ROWS} listed batters "
        "(PropFinder omitted the rest)"
    )
if ZONE_ROW_COUNT < ZONE_ROW_MIN:
    raise SystemExit(
        f"GAMES_BLOCK missing zone data ({ZONE_ROW_COUNT} zoneScore fields; "
        f"{ZONE_ELIGIBLE} batters have zone rows, need {ZONE_ROW_MIN}+)"
    )
...
def sp_throws_from_title(title: str, chip: str) -> str | None:
    """L/R for the starter a chip names, parsed out of the game title.

    Switch hitters bat opposite the arm, so the platoon lane cannot be chosen
    from the batter's hand alone. The title carries both starters as
    "Name (R, TEAM)", which is the same fact the header prints.
    """
    ckey = re.sub(r"[^a-z]", "", (chip or "").lower())
    if not ckey:
        return None
    for seg in title.split(" - ", 1)[-1].split(" vs "):
        m = re.search(r"\((L|R),", seg)
        if not m:
            continue
        name = seg[: m.start()].replace("🧤", "").strip()
        nkey = re.sub(r"[^a-z]", "", name.lower())
        last = re.sub(r"[^a-z]", "", name.split()[-1].lower()) if name.split() else ""
        if nkey.endswith(ckey) or (last and ckey.endswith(last)):
            return m.group(1)
    return None
...
def collect_rows():
    ...
    for g in ENRICHED_GAMES:
        ...
        for r in g["rows"]:
            ...
            chip_throws = sp_throws_from_title(g["title"], chip)
            ...
            if risk_row:
                split = batter_split(hand, risk_row, chip_throws) or 0.0
...
def starting_pitchers_today() -> set[str]:
    """Last-name keys of the arms actually starting, taken from the game titles."""
    keys: set[str] = set()
    for g in ENRICHED_GAMES:
        title = g["title"]
        if " - " not in title or " vs " not in title:
            continue
        away_seg, home_seg = title.split(" - ", 1)[1].split(" vs ", 1)
        for seg in (away_seg, home_seg):
            name = seg.rsplit(" (", 1)[0].replace("🧤", "").strip()
...
def load_pitchers_to_attack():
    """Attackable arms — restricted to pitchers who are actually starting today.

    hr-targets-overall keeps whatever probable PropFinder had when it built the
    export. On 2026-08-18 that meant Jack Perkins sat second on this card at 0.92
    HR risk while Brady Basso was the arm actually taking the ball. Recommending a
    pitcher who is not pitching is worse than showing four names, so filter.
    """
    starting = starting_pitchers_today()
    dropped: list[str] = []
    rows = []
    for row in PITCHER_RISK.values():
        name = row["pitcher"]
        if starting and name.lower() not in starting and name.split()[-1].lower() not in starting:
            dropped.append(name)
            continue
        rows.append(
            {
    ...
    if dropped:
        print(f"pitchers-to-attack: dropped {len(dropped)} non-starter(s): {', '.join(sorted(dropped))}")
...
# ── who is actually in a posted lineup ────────────────────────────────────────
# annotate_hits_ranks() stamps lineup_slot, but it runs far below this point --
# long after the straights and Goblin legs are chosen. Read the lineups here so
# every HR board can exclude a bat that is not starting.
def _ensure_research_json() -> None:
    """Build the research JSON before anything reads lineups from it.

    This is the 8/20 ordering trap again, reintroduced by the starting-lineup
    guard: post_patch writes research-DATE.json at the END of this script, but
    _mark_starting reads it near the top. On a first-try run the file does not
    exist yet, _mark_starting marks every hitter as starting, and the guard is
    silently inert -- which is how a slot-14 catcher reached the 2-Leg card.
    Earlier slates only looked fine because the patch had been re-run.
    """
    # Always refresh, not just when the file is absent. A stale snapshot is the same
    # bug wearing a different hat: the boards get picked off yesterday's lineup card
    # and the page ships today's. Taking the snapshot here, once, means every pick
    # below and the research tab at the end are reading the identical lineups.
    # An audit executes this file's top half to inspect the boards it picks. It must
    # READ the snapshot, never retake it: re-fetching there rewrote the lineups after
    # the page had already been built from the previous ones, which is the exact drift
    # the snapshot exists to prevent -- on 9/16 the file landed 73 seconds after the
    # page. Audits set this flag; a real patch run never does.
    import os as _os

    path = ROOT / "preview" / "data" / f"research-{SHEET_DATE}.json"
    if _os.environ.get("WORSTPICKZ_AUDIT_READONLY") and path.is_file():
        return
    print(f"taking the lineup snapshot for {SHEET_DATE} before any board is picked")
    from research.sync_tab import refresh_research_tab as _prebuild

    _prebuild(SHEET_DATE, with_stats=True, update_meta=False)


def _mark_starting(all_rows: list[dict]) -> int:
    from goblin_hits_parlay import (
        LAST_STARTING_SLOT,
        load_research_hit_stats,
        lookup_research_entry,
    )

    stats = load_research_hit_stats(SHEET_DATE)
    if not stats:
        raise SystemExit(
            f"no research lineups for {SHEET_DATE} — refusing to pick boards blind "
            "(the guard would silently pass every benched bat)"
        )
    benched = 0
    for r in all_rows:
        rec = lookup_research_entry(stats, r["name_plain"], r.get("game")) or {}
        slot = rec.get("lineup_slot")
        starting = slot is not None and int(slot) <= LAST_STARTING_SLOT
        r["is_starting"] = starting
        if not starting:
            benched += 1
    return benched


_ensure_research_json()
_benched = _mark_starting(rows)
if _benched:
    print(f"lineups: {_benched} listed prop(s) not in a posted lineup — kept on the board, barred from picks")

listed_rows = [r for r in rows if r["odds_value"] is not None]
if not listed_rows:
    listed_rows = rows
# Straights/Goblin HR legs: include N/A odds props (user list); listed-only for
# longshots display. Anyone not in a posted lineup is excluded from picking --
# he still renders as a board row, he just cannot be a leg.
straight_rows = [r for r in rows if r.get("is_starting", True)]
...
def pick_top_n(
    ...
    *,
    ...
    game_caps: dict[str, int] | None = None,
) -> list[dict]:
    """Pick top rows with at most max_per_game from the same game (and team when set).

    game_caps raises the cap for specific games only, so an extreme park can be
    stacked without loosening the two-per-game rule across the rest of the slate.
    """
    ...
    for r in ranked:
        ...
        if game_count >= (game_caps or {}).get(r["game_key"], max_per_game):
            continue
        if max_per_team is not None and r.get("team"):
...
def straight_o05_pool(
    ...
    *,
    ...
    pool = [
        ...
        and r["name"] not in RESERVED_FOR_FAV3
        # The Straight of the Day is the sheet's headline bet, so it has to be
        # bettable. Rows PropFinder shipped without a price render as "Listed prop -"
        # with no number, which is no use to anyone tailing it.
        and (r.get("odds_value") or 0) > 0
...
# The Worst Pickz Favorite 3-leg can only be built from ⭐ rows that clear the lane
# gates, and on some slates that is exactly three names (8/17: Schwarber, Conforto,
# Murakami — every other favorite sits on a harsh negative platoon). Straights are
# picked first, so without this the straight would eat one and the Favorite 3-leg
# could not fill. Reserve them only when the board is that thin; on a normal slate
# with four or more usable favorites this is empty and straights pick freely.
FAV3_ELIGIBLE = {
    r["name"]
    for r in rows
    if r["name"] in FAVS
...
RESERVED_FOR_FAV3 = set(FAV3_ELIGIBLE) if len(FAV3_ELIGIBLE) <= 3 else set()
if RESERVED_FOR_FAV3:
    print(
        f"thin favorite board ({len(FAV3_ELIGIBLE)} usable ⭐): reserving "
        f"{', '.join(sorted(RESERVED_FOR_FAV3))} for the Favorite 3-leg"
    )
...
# Score gates below are percentile-matched to the pre-2026-09-23 scale. The rating
...
def weather_play_ok(row: dict) -> bool:
    ...
    # Form on THIS branch has to be measured, not inferred from the composite. The
    # score at a +35% park is largely the park, so "big park" plus "high score" is
    # one fact counted twice -- which is how a lefty with no home run, one near-miss
    # and a -0.76 lane against a lefty rode a park number onto the card. Real HR
    # evidence only: a home run, or two near-misses.
    if park >= 35 and row["score"] >= 81 and (row["hr"] >= 1 or row["near"] >= 2):
        return True
...
# Ranking alone does not protect the Favorite 3-leg: the top three ⭐ by attack rank
# can include a name the fav3 lane gates reject (8/17: Encarnacion-Strand at -0.58),
# which leaves a genuinely usable favorite unreserved for the Goblin 3-leg to take.
# Union in the gate-eligible set so every favorite the 3-leg can use is held back.
reserved_fav_names = {r["name"] for r in fav_reserve} | RESERVED_FOR_FAV3
...
# The owner allows the Favorite 3-leg to reuse ONE of the day's two straights. Keep
# the better-ranked straight eligible and bar the other, so the card can double down
# on the strongest favorite without simply reprinting the straights.
fav3_reuse_pool = sorted(
    (
        r
        for r in rows
...
FAV3_REUSABLE = {fav3_reuse_pool[0]["name"]} if fav3_reuse_pool else set()
fav3_blocked_straights = straight_names - FAV3_REUSABLE
if FAV3_REUSABLE:
    print(f"Favorite 3-leg may reuse the straight: {', '.join(sorted(FAV3_REUSABLE))}")
...
fav_pool = [
    ...
    and r["name"] not in {x["name"] for x in top3}
    and r["name"] not in {x["name"] for x in two_leg}
    and r["name"] not in fav3_blocked_straights
    and fav3_lane_ok(r)
    and (r["risk"] >= 0.0 or r["split"] >= 0.50)
    and fav_secondary_lane_ok(r)
]
...
fav3 = pick_top_n(
    fav_pool,
    3,
    exclude_names={x["name"] for x in top3} | {x["name"] for x in two_leg} | fav3_blocked_straights,
    max_per_game=2,
    max_per_team=2,
)
if len(fav3) < 3:
...
if len(fav3) < 3:
    ...
    fav3.extend(
        pick_top_n(
            fav_fill,
            3 - len(fav3),
            exclude_names={x["name"] for x in top3} | {x["name"] for x in two_leg} | fav3_blocked_straights | {x["name"] for x in fav3},
            max_per_game=2,
            max_per_team=2,
        )
    )
...
# Backfill when the strict pool cannot fill three seats. This used to also require
# len(rows) <= 30 -- but board size is not what makes a favorite board thin. Today
# there are 69 props and only two usable stars, because most of this slate's arms
# grade negative on HR risk, so the backfill never fired and the patch died on the
# assert. The shortfall itself is the condition.
if len(fav3) < 3:
    fav_thin = sorted(
        [
            r
...
assert not ({x["name"] for x in top3} & {x["name"] for x in fav3}), "3 Leg and Favorite 3 Leg must be distinct"
# The owner allows the Favorite 3-leg exactly ONE straight, no more: without a
# ceiling the card could quietly become a reprint of the day's two headline bets.
assert len({x["name"] for x in fav3} & straight_names) <= 1, (
    "Favorite 3-leg may reuse at most one Straight of the Day"
)
assert not ({x["name"] for x in top3} & {x["name"] for x in two_leg}), "3 Leg and 2 Leg must be distinct"

# Hits parlay selector (max 11 legs): BIP% + zone fit first, contact form second.
# The hits rubric ranks on lineup slot -> expected PA, and that lives in the research
# JSON. post_patch writes that file at the END of this script, so on the FIRST run for
# a new date it does not exist yet: load_research_hit_stats() returns {}, every hitter
# falls back to DEFAULT_EXPECTED_PA, and the whole rubric goes inert. It shipped that
# way on 8/18-8/20 -- the 8/20 ticket had two bench bats (slots 11 and 12) on it.
# Build the research JSON before ranking, not after.
_research_json = ROOT / "preview" / "data" / f"research-{SHEET_DATE}.json"
# _ensure_research_json() ran at the top, so this should never fire. It stays as a
# tripwire: if it ever does, something deleted the snapshot mid-run.
if not _research_json.is_file():
    print(f"research JSON missing for {SHEET_DATE} — building it before ranking hits legs")
    from research.sync_tab import refresh_research_tab as _prebuild_research

    _prebuild_research(SHEET_DATE, with_stats=True, update_meta=False)

annotate_hits_ranks(rows, row_high_whiff=row_high_whiff, sheet_date=SHEET_DATE)
if not any(r.get("lineup_slot") for r in rows):
    raise SystemExit(
        f"hits rubric has no lineup slots for {SHEET_DATE} — research JSON is missing or "
        "empty, so expected-PA ranking would silently fall back to league defaults"
    )
...
def assert_game_cap(
    ...
    bad = {
        ...
        if count > (game_caps or {}).get(game, max_per_game)
    }
    if bad:
...
# The 8/31 lineup guard reached the straights and Goblin legs but never these three
# summary cards, so a bat with no posted lineup could still be recommended here.
# Same rule: he stays a board row, he just cannot be a recommendation.
top5_quality = [r for r in straight_rows if summary_ticket_ok(r)]
...
# candidate list left the shortfall backfill further down reading weather_merged
# directly, so a benched bat could still land on the card.
weather_merged: dict[str, dict] = {}
for r in weather_primary + weather_relaxed + weather_fill:
    if not r.get("is_starting", True):
        continue
...
# Gate the stacking allowance on the park each hitter actually plays into. ATH shows
# +25% at the venue level today but only +10% RHB / +1% LHB, so it is not a Coors-style
# slate and the chart stays at the standard two-per-game cap.
boosted_weather_games = {
    r["game_key"] for r in weather_merged.values() if effective_hand_park_pct(r) >= 5
}
extreme_weather_games = {
    r["game_key"] for r in weather_merged.values() if effective_hand_park_pct(r) >= 20
}
coors_slate = any(effective_hand_park_pct(r) >= 28 for r in weather_merged.values())
# The stacking allowance exists for a single standout park, so scope it to that game.
# Applying it as a global cap let an ordinary +10% park put three bats on the chart,
# which breaks the two-per-game rule the rest of the board follows.
...
weather_game_caps = {gk: weather_stack_cap for gk in extreme_weather_games}
if coors_slate:
    weather_game_caps.update(
        {
            r["game_key"]: max(weather_stack_cap, weather_game_caps.get(r["game_key"], 0))
            for r in weather_merged.values()
            if effective_hand_park_pct(r) >= 28
        }
    )
weather5 = pick_top_n(
    weather_candidates,
...
if len(weather5) < 5:
    ...
    for r in fill_pool:
        ...
        if game_count >= weather_game_caps.get(r["game_key"], weather_max_per_game):
            continue
        weather5.append(r)
        have.add(r["name"])
        if len(weather5) == 5:
...
longshot_pool = [r for r in listed_rows if r.get("is_starting", True) and longshot_ok(r)]
                    ...
                    <div class="straight-picks-grid">
                        ...
                        <div class="straight-pick-hero straight-pick-hero--o15">
                            ...
                            <div class="straight-pick-header">
                                ...
                                <span class="straight-pick-meta">Over 1.5 HR &middot; Score {straight_o15['score']} &middot; {straight_o15['game_key']}</span>
                            </div>
                            <ul class="straight-pick-factors">
...
# The card is capped at five, not guaranteed five. When the parks do not support
# five qualifying bats -- today eight of ten play negative -- the heading counts what
# is actually there. Saying "Top 5" above two rows reads as content that failed to
# load, which is the same false alarm an empty card raises; the honest shortage
# should look like a shortage, not like a bug. Filling the slots instead would mean
# promoting bats that failed the gate, which is the opposite of what the card is for.
# Say WHY the card is short, and only say what is true. On 9/14 the reason really was
# the parks -- eight of ten played negative. On 9/16 it was not: three more Coors bats
# cleared the weather gate and were held back by the two-per-game cap, so "parks
# support no more" would have been false on its face.
def _weather_shortfall_reason() -> str:
    _picked = {r["name"] for r in weather5}
    _t5 = {r["name"] for r in top5}
    _count = {}
    for r in weather5:
        _count[r["game_key"]] = _count.get(r["game_key"], 0) + 1
    capped = benched = 0
    for r in rows:
        if r["name"] in _picked or r["name"] in _t5 or not weather_play_ok(r):
            continue
        if not r.get("is_starting", True):
            benched += 1
        elif _count.get(r["game_key"], 0) >= weather_game_caps.get(r["game_key"], weather_max_per_game):
            capped += 1
    if capped:
        return f" (more qualify, capped at {weather_max_per_game} per game)"
    return " (parks support no more today)"


WEATHER5_HEADING = (
    "Top 5 Weather Heavy HR Plays"
    if len(weather5) >= 5
    else f"Top {len(weather5)} Weather Heavy HR Play"
    + ("s" if len(weather5) != 1 else "")
    + _weather_shortfall_reason()
)
...
def archive_relative_assets(text: str) -> str:
    """Archives live one level down in preview/archive/, so asset paths must climb."""
    for attr in ("src", "href"):
        for quote in ('"', "'"):
            text = text.replace(
                f"{attr}={quote}assets/", f"{attr}={quote}../assets/"
            )
    return text
...
def sheet_label(date: str) -> str:
    y, m, d = date.split("-")
    return f"{MONTH_NAMES[int(m) - 1]} {int(d)}, {y}"


def update_manifest():
    """Rebuild the sheet picker from what is actually on disk.

    This used to be a hand-maintained list of ~60 literal dates plus a parallel
    chain of `elif date == "..."` fallbacks. Every new slate meant editing the
    same date in five places, and missing one silently dropped an archive from
    the picker or mislabelled the current slate. Deriving it from
    preview/archive/*.html keeps the picker correct without per-day edits.
    """
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    known = {sheet["date"]: sheet for sheet in manifest.get("sheets", [])}

    archive_dir = ROOT / "preview" / "archive"
    archived = {p.stem for p in archive_dir.glob("20??-??-??.html")}
    # The slate being replaced becomes an archive as part of this patch run.
    if ARCHIVE_PREVIOUS.is_file():
        archived.add(ARCHIVE_PREVIOUS.stem)

    ordered = [
        {
            "date": SHEET_DATE,
            "label": f"{sheet_label(SHEET_DATE)} — current slate",
            "href": "index.html",
        }
    ]
    for date in sorted(archived - {SHEET_DATE}, reverse=True):
        entry = known.get(date)
        # Keep any hand-tuned label, but never keep a stale "current slate" tag.
        label = (entry or {}).get("label", "")
        if not label or "current slate" in label:
            label = sheet_label(date)
        ordered.append({"date": date, "label": label, "href": f"archive/{date}.html"})

    missing = sorted(set(known) - {e["date"] for e in ordered})
    if missing:
        print(f"manifest dropped {len(missing)} entry(s) with no archive on disk: {', '.join(missing)}")
    added = sorted(archived - set(known) - {SHEET_DATE})
    if added:
        print(f"manifest added {len(added)} archive(s): {', '.join(added)}")
...
def patch_preview(manifest):
    ...
    # Derived from SHEET_DATE, never hardcoded: this line was the one piece of the
    # intro copy that did not update itself, so a copied patch script shipped the
    # previous slate's weekday and date in the hero while every computed count around
    # it was correct.
    hero_date = datetime.date.fromisoformat(SHEET_DATE).strftime("%A, %B %d, %Y")
    hero_date = hero_date.replace(" 0", " ")  # "August 07" -> "August 7"
    text = re.sub(
        r"<p>(?:Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday), \w+ \d+, \d{4} — Worst Pickz HR cheat sheet",
        lambda _m: f"<p>{hero_date} — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
...
def main():
    ...
    if preview_date == archive_date:
        ...
        ARCHIVE_PREVIOUS.write_text(
            archive_relative_assets(PREVIEW.read_text(encoding="utf-8")),
            encoding="utf-8",
        )
    ...
    apply_hidden_gem_ui()
    sync_research_tab_after_patch(SHEET_DATE, reuse_existing=True)
