#!/usr/bin/env python3
"""List all straight options for 2026-06-27 by Split/Risk/Park/Form/Zone."""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parent
cut = (ROOT / "patch-0627-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
g: dict = {"__file__": str(ROOT / "patch-0627-preview.py"), "__name__": "__main__"}
exec(compile(cut + "\n", "patch-0627-preview.py", "exec"), g)

from goblin_hr_zone_fit import hr_power_form, zone_hr_fit
...
def fmt_row(r: dict, rank: float | None = None) -> str:
...
print("=" * 100)
print("CURRENT STRAIGHTS ON SHEET")
print("=" * 100)
...
print("\n" + "=" * 100)
print("ALL O0.5 OPTIONS — strict pool (sheet rubric: score>=72, split>=0, zone lane ok)")
print("=" * 100)
...
print("\n" + "=" * 100)
print("ALL O0.5 OPTIONS — relaxed pool (positive split, usable form)")
print("=" * 100)
o05_relaxed = [
    r
...
print("\n" + "=" * 100)
print("ALL O0.5 BY SPLIT+RISK+PARK+FORM+ZONE (composite rank, top 20)")
print("=" * 100)
o05_comp = sorted(
...
print("\n" + "=" * 100)
print("ALL O1.5 OPTIONS — strict pool (2+ HR, 2+ near, score>=78, positive split)")
print("=" * 100)
...
print("\n" + "=" * 100)
print("VALID O0.5 + O1.5 PAIRS (different games) — top combos by combined composite")
print("=" * 100)
...
for o15 in o15_relaxed[:12]:
    for o05 in o05_comp[:15]:
        if o05["game_key"] == o15["game_key"]:
            continue
        if o05["name"] == o15["name"]:
            continue
        pairs.append((composite(o05) + composite(o15), o05, o15))
pairs.sort(key=lambda x: x[0], reverse=True)
for i, (score, o05, o15) in enumerate(pairs[:12], 1):
