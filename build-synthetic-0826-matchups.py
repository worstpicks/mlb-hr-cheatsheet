#!/usr/bin/env python3
"""Synthesize the hr-matchups game PropFinder did not export for 2026-08-26.

  NYY  Gerrit Cole -> Elmer Rodriguez   MLB still lists New York as TBD; the sheet's
                          owner named Rodriguez and exported Cole as a pitch-mix
                          stand-in. Batter rows come from the Cole export -- same
                          Houston lineup, and batter form does not depend on who is
                          pitching. Cole's pitcher block is NOT carried: those are
                          Cole's measured numbers and would read as Rodriguez's.
                          Rodriguez has a real 2026 MLB book (17.0 IP, .284 BAA,
                          0.53 HR/9), so his header renders those measured rates.
"""
...
GAPS: dict[str, dict] = {
    "Elmer Rodriguez": {
        ...
        "oteam": "HOU",
        "lineups": "sibling",
        "sibling": f"hr-matchups-HOU-at-NYY-Gerrit-Cole-{DATE}.csv",
        "block_from": None,
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-HOU-at-NYY-Gerrit-Cole-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 8/26 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# Nothing is carried. There is no prior PropFinder score for Elmer Rodriguez to carry
# forward, and Cole's row is Cole's -- putting it under Rodriguez's name would be a
# fabrication, not a stand-in. His real measured rates cover the header instead.
