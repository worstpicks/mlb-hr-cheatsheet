#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-13. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-08-12.html"
SHEET_DATE = "2026-08-13"

spec = importlib.util.spec_from_file_location("build0813", ROOT / "build-sheet-2026-08-13.py")
...
# 8/13 judgment: O0.5 Teoscar Hernandez vs Drohan — the top-ranked lane with nothing
# working against it: 3rd-best zone fit (37.2) carrying the board's best zone hard-hit
# (59.1), 2 HR/3 near-HR at 91 EV and heating, +0.49 split as a RHB against a LHP, and
# +10% RHB park at Dodger Stadium. Reject Schwarber despite a similar zone score: Bradley
# is not attackable (-0.16) and Field of Dreams grades -10%. Reject Ben Rice despite the
# slate's biggest park edge (+20% LHB): 0 HR and 0 near-HR is no form to ride. Abreu owns
# the best matchup on the slate (+1.54 into Scherzer 🧤) but his 24.4 zone fit trails the
# field badly, and this rubric leads with zone — he anchors the 3-leg instead.
# O1.5 Owen Caissie vs Ashcraft — slate-best zone fit (41.4) and barrel zone (44.5) with
# 2 HR/2 near-HR at 94 EV, heating, on a +0.45 split into a +0.46-risk arm. Conine has the
# louder raw form (3 HR/4 near, 102 EV) but the zone lead is Caissie's and the rubric leads
# with zone; both sit in PIT @ MIA so only one can carry a straight.
_teoscar = next((r for r in straight_rows if r["name_plain"] == "Teoscar Hernandez"), None)
_caissie = next((r for r in straight_rows if r["name_plain"] == "Owen Caissie"), None)
if _teoscar is not None:
    straight_o05 = _teoscar
if _caissie is not None and _caissie["game_key"] != straight_o05["game_key"]:
    straight_o15 = _caissie
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/13 Goblin judgment: distinct lanes after the straights; reserve the ⭐ bats for Fav3.
# 3 Leg — Wilyer Abreu (top non-favorite attack lane: +1.54 split into Scherzer 🧤, the
# slate's worst arm at 2.75 HR/9 and 2.82 vs LHB, +8% LHB park at Rogers Centre, 2 HR/2
# near at 92 EV and heating), Nathaniel Lowe (zone 30.8 with a 27.4 zone-HR, 1 HR/2 near at
# 94 EV, heating), Mookie Betts (+0.49 into Drohan, +10% park, 1 HR/2 near at 94 EV,
# heating). Three different games, every leg with real HR form behind it.
# 2 Leg — Jarren Duran (the other side of the Scherzer 🧤 lane: +1.54 split, +8% park, 98
# EV) and William Contreras (best split among the leftovers at +0.91, +10% park, heating
# 1 HR/2 near). Spencer Jones and Grisham skipped: the +20% Yankee Stadium park is real but
# neither has an HR in the window.
# Fav 3 — Chourio (+0.91 split, +10% park, 2 HR/3 near at 99 EV), Conine (loudest form on
# the slate: 3 HR/4 near at 102 EV into a +0.46-risk arm), Clemens (+1.05 split into Nola
# 🧤 at 1.23 risk). Schwarber and Rice rejected: top attack ranks but no HR form and a
# negative park/risk lane behind them.
_top3_lock_names = ["Wilyer Abreu", "Nathaniel Lowe", "Mookie Betts"]
_two_lock_names = ["Jarren Duran", "William Contreras"]
_fav3_lock_names = ["Jackson Chourio", "Griffin Conine", "Kody Clemens"]
...
def update_manifest():
    ...
    if "2026-08-12" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-08-12"] = {
            "date": "2026-08-12",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 13, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-12",
        "2026-08-11",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Thursday, August 13, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
