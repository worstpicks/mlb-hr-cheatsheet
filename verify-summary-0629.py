...
patch_globals: dict = {"__file__": str(ROOT / "patch-0629-preview.py"), "__name__": "__main__"}
cut = (ROOT / "patch-0629-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
exec(compile(cut + "\n", "patch-0629-preview.py", "exec"), patch_globals)
...
top5_expected = min(5, len(patch_globals["rows"]))
longshot_min = (
    1
    if any(
...
for label, picked, ok_fn in (
    ...
    for r in picked:
        if label == "Top 5 HR Tickets" and len(patch_globals["rows"]) <= 16:
            continue
        if not ok_fn(r):
            errors.append(
...
if errors:
    print("FAIL verify-summary-0629:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK verify-summary-0629")
