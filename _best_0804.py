...
ROOT = Path(".")
spec = importlib.util.spec_from_file_location("b", "build-sheet-2026-08-04.py")
b = importlib.util.module_from_spec(spec)
spec.loader.exec_module(b)
risk = load_pitcher_risk(ROOT / "data/hr-targets-overall-2026-08-04.csv")
rows = []
for g in enrich_games_list(b.games, "2026-08-04"):
        ...
        rows.append(
            {
                ...
                "fav": "⭐" in "".join(r.get("emojis") or []),
                "gem": "💎" in "".join(r.get("emojis") or []),
...
o05 = [
    r
    for r in rows
    if o05_zone_lane_ok(r)
    and r["split"] >= 0
    and not (r["split"] <= 0 and r["risk"] <= 0)
]
...
for r in o05[:12]:
    print(
        ...
        f"park={r['park_pct']:+d} HR={r['hr']}/{r['near']} fav={r['fav']}"
    )
o15 = sorted(
...
for r in o15[:10]:
    print(
        f"  {r['name']:22} vs {r['chip']:20} multi={r['multi_hr_rank']:5.1f} "
        f"split={r['split']:+.2f} risk={r['risk']:+.2f} park={r['park_pct']:+d} "
        f"HR={r['hr']}/{r['near']} fav={r['fav']}"
    )
print("KEY NAMES:")
for n in [
    "Bryce Harper",
    ...
    print(
        f"  {n:22} atk={r['straight_attack_rank']:5.1f} multi={r['multi_hr_rank']:5.1f} "
        f"z={r['hr_zone_fit']:5.1f} split={r['split']:+.2f} risk={r['risk']:+.2f} "
        f"park={r['park_pct']:+d} HR={r['hr']}/{r['near']}"
    )
