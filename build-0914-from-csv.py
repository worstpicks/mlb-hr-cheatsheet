#!/usr/bin/env python3
"""Build 2026-09-14 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-14"
OUT = ROOT / "build-sheet-2026-09-14.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0914-matchups.py. Nothing is carried today: every arm the sheet
...
BATTER_GAME_OVERRIDES: dict[str, str] = {
    # Two Max Muncys again. The list settles it inline this time -- "Max Muncy(LAD)"
    # -- so the Dodger is pinned and Oakland's is not on the board.
    "Max Muncy": "LAD @ CIN",
}
...
PROBABLE_OVERRIDES: dict[str, dict] = {
    # PropFinder and MLB both list Sean Newcomb for Chicago; the owner's pasted
    # block names Sean Burke and tags him PRIM. The workload settles it rather than
    # taste: Newcomb has 5 starts at 1.8 IP and 7.0 batters apiece -- an opener --
    # while Burke has 25 starts at 5.2 IP and 22.3 batters, which is the 8-7 / 3.58
    # line the owner pasted. The bats in this game will see Burke, so the sheet
    # names Burke.
    #
    # This flips the platoon math: Newcomb throws left, Burke throws right, so every
    # Cleveland hitter's lane is recomputed against a right-hander rather than
    # inherited. Burke is absent from PropFinder's summaries, so his header renders
    # measured MLB/Savant rates instead of the export's relative percentages.
    "CWS @ CLE": {
        "side": "away",
        "from": "Newcomb",
        "to": "Burke",
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-14 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-14 sent "Alejanro Kirk", which matched nothing at
