#!/usr/bin/env python3
"""Build 2026-09-23 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-23"
OUT = ROOT / "build-sheet-2026-09-23.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0923-matchups.py. Nothing is carried today: every arm the sheet
...
# Toronto at Baltimore is a doubleheader: rain postponed 9/22 and it was made up as
# game one today. PropFinder exported game one only; game two's two files are
# gap-filled by build-synthetic-0923-matchups.py (see its docstring).
DOUBLEHEADER_SPECS: dict[str, dict[str, dict]] = {
    "TOR @ BAL": {
        "TOR @ BAL (G1)": {
            "away_sp_full": "Max Scherzer",
            "home_sp_full": "Chris Bassitt",
            "files": [
                f"hr-matchups-TOR-at-BAL-Max-Scherzer-{DATE}.csv",
                f"hr-matchups-TOR-at-BAL-Chris-Bassitt-{DATE}.csv",
            ],
        },
        "TOR @ BAL (G2)": {
            "away_sp_full": "CJ Van Eyk",
            "home_sp_full": "Trey Gibson",
            "files": [
                f"hr-matchups-TOR-at-BAL-CJ-Van-Eyk-{DATE}.csv",
                f"hr-matchups-TOR-at-BAL-Trey-Gibson-{DATE}.csv",
            ],
        },
    },
}
...
PITCHER_HAND = {
    # 2026-09-23: Fedde (PRIM, over the opener Hudson) and the two game-two arms.
    "Erick Fedde": "R",
    "CJ Van Eyk": "R",
    "Trey Gibson": "R",
    # 2026-09-23: Bradford replaces the opener Alexander (see PROBABLE_OVERRIDES);
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-23 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-23 sent "Alejanro Kirk", which matched nothing at
