#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-10. Does not commit or push."""
...
spec = importlib.util.spec_from_file_location("build0810", ROOT / "build-sheet-2026-08-10.py")
...
# 8/10 judgment: O0.5 Burger vs Detmers 🧤 (atk/zone lead).
# O1.5 Merrill vs Henderson (2/2) — different game (Siri same game as Burger).
_burger = next((r for r in straight_rows if r["name_plain"] == "Jake Burger"), None)
_merrill = next((r for r in straight_rows if r["name_plain"] == "Jackson Merrill"), None)
if _burger is not None:
    straight_o05 = _burger
if _merrill is not None and _merrill["game_key"] != straight_o05["game_key"]:
    straight_o15 = _merrill
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/10 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Derek Hill", "Wilyer Abreu", "Brandon Nimmo"]
_two_lock_names = ["Coby Mayo", "Jackson Chourio"]
_fav3_lock_names = ["Pete Alonso", "Jose Siri", "Willson Contreras"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 10, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-09",
        "2026-08-08",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Monday, August 10, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
