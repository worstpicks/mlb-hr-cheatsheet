#!/usr/bin/env python3
"""Patch preview sheet to 2026-09-01. Does not commit or push."""
...
spec = importlib.util.spec_from_file_location("build0901", ROOT / "build-sheet-2026-09-01.py")
...
def _fold_name(n: str) -> str:
    ...
    return _re.sub(r"[^a-z0-9]", "", b.lower())


def _zone_covered_pairs() -> set[tuple[str, str]]:
    """(batter, opposing pitcher) pairs zone-matchups actually shipped.

    Coverage is per matchup, not per batter: in a doubleheader PropFinder keys the
    zone export to game one's arms, so game two's batters -- the same people -- have
    no zone row for the pitcher they actually face.
    """
    import csv as _csv

    path = ROOT / "data" / f"zone-matchups-{SHEET_DATE}.csv"
    if not path.is_file():
        return set()
    with path.open(encoding="utf-8-sig") as fh:
        return {(_fold_name(r["Batter"]), _fold_name(r["Pitcher"])) for r in _csv.DictReader(fh)}


_covered = _zone_covered_pairs()
ZONE_ELIGIBLE = 0
for _g in build.games:
    _sps = _g["title"].split(" - ", 1)[1].replace(" 🧤", "")
    for _r in _g["rows"]:
        _vs = (_r.get("chips") or [""])[0].replace("vs ", "").strip()
        _nm = _fold_name(_r["name"].rsplit(" (", 1)[0])
        if any(_nm == b and _vs and _fold_name(_vs) in pk for b, pk in _covered):
            ZONE_ELIGIBLE += 1
...
def _mark_starting(all_rows: list[dict]) -> int:
    from goblin_hits_parlay import LAST_STARTING_SLOT, load_research_hit_stats
    from goblin_hits_parlay import _norm_name as _norm

    stats = load_research_hit_stats(SHEET_DATE)
    if not stats:
        for r in all_rows:
            r["is_starting"] = True  # no lineup data: do not silently drop the board
        return 0
    benched = 0
    for r in all_rows:
        rec = stats.get(_norm(r["name_plain"])) or {}
