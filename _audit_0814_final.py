#!/usr/bin/env python3
"""Final local audit for 2026-08-14 (do not push)."""
...
def main() -> int:
    ...
    if "Friday, August 14, 2026" not in html:
        fail("hero must be Friday, August 14, 2026")
    if "Thursday, August 13, 2026 — Worst" in html:
        fail("stale 8/13 hero still on current sheet")
    if "Thursday, August 14" in html or "Saturday, August 14" in html:
        fail("wrong weekday on August 14 (must be Friday)")
    if "August 14, 2026 — current slate" not in html:
        fail("manifest missing August 14 current slate")
    if "August 13, 2026 — current slate" in html:
        fail("8/13 still labeled current")
    ...
    expected_bum = {"Grayson Rodriguez", "Gavin Williams", "Kumar Rocker"}
    ...
    arch = ROOT / "preview" / "archive" / "2026-08-13.html"
    if not arch.exists():
        fail("8/13 archive missing")
    elif 'content="2026-08-13"' not in arch.read_text(encoding="utf-8"):
        fail("8/13 archive wrong sheet-date")
