#!/usr/bin/env python3
"""Patch preview sheet to 2026-06-18. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-06-17.html"
SHEET_DATE = "2026-06-18"

spec = importlib.util.spec_from_file_location("build0618", ROOT / "build-sheet-2026-06-18.py")
...
ZONE_ROW_MIN = max(45, TOTAL_ROWS - 12)
...
# 6/18 judgment: O0.5 Soto (+25% park, +0.52 split) + O1.5 Montgomery (2 HR/4 near vs Weathers 🧤).
rows_by_plain_early = {r["name_plain"]: r for r in rows}
_j_o15 = rows_by_plain_early.get("Colson Montgomery")
_j_o05 = rows_by_plain_early.get("Juan Soto")
if (
    _j_o15
    and _j_o05
    and _j_o15["hr"] >= 2
    and _j_o05["game_key"] != _j_o15["game_key"]
    and _j_o05["split"] >= 0.0
    and effective_park_pct(_j_o05) >= 5
    and _j_o15["split"] >= 0.0
):
    straight_o15 = _j_o15
    straight_o05 = _j_o05
    straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 6/18 Favorite 3-leg: all three user ⭐ on this slate (reserve for Favorite 3 Leg, not 3 Leg HR).
_fav3_pick_order = ["Randal Grichuk", "Jordan Walker", "Drake Baldwin"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "June 18, 2026 — current slate", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Thursday, June 18, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
