#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-16 NYM @ PHI. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-14.html"
SHEET_DATE = "2026-07-16"

spec = importlib.util.spec_from_file_location("build0716", ROOT / "build-sheet-2026-07-16.py")
...
# Thin single-game slate: allow stacking beyond the usual 2/game diversification cap.
THIN_SLATE = TOTAL_GAMES <= 1
GAME_CAP = 5 if THIN_SLATE else 2
TEAM_CAP = None if THIN_SLATE else 2
# Same-game O0.5 / O1.5 is required when there is only one game.
STRAIGHT_EXCLUDE_GAME = False if THIN_SLATE else True
...
if straight_o05["game_key"] == straight_o15["game_key"] and STRAIGHT_EXCLUDE_GAME:
    alt_o15 = next(
...
# Thin NYM@PHI board: lock Favorite 3 before Goblin 3-leg so ⭐ are not consumed.
rows_by_plain = {r["name_plain"]: r for r in rows}
_early_fav3: list[dict] = []
for _nm in ("Bryce Harper", "Juan Soto", "Francisco Lindor", "Brett Baty", "Edmundo Sosa"):
    ...
    if _r["name"] not in FAVS and favs_seated < min(2, len(FAVS)):
        continue
    if _r["name"] in {x["name"] for x in _early_fav3}:
        continue
    _early_fav3.append(_r)
    if len(_early_fav3) == 3:
        break
_early_fav3_names = {x["name"] for x in _early_fav3}
...
def goblin_hr_leg_ok(row: dict) -> bool:
    if row["hr"] < 1 and row["near"] < 2:
        # ASG / extreme CBP weather: strong platoon + park can carry thin event form.
        if not (
            THIN_SLATE
            and row["score"] >= 74
            and row["split"] >= 0.50
            and park_gate_pct(row, park_pct_fn=effective_park_pct) >= 20
        ):
            return False
...
top3_pool = [
    ...
    and r["name"] not in _early_fav3_names
...
reserved_fav_names = {r["name"] for r in fav_reserve} | _early_fav3_names

top3_pool_reserved = [r for r in top3_pool if r["name"] not in reserved_fav_names]
top3 = pick_top_n(top3_pool_reserved, 3, exclude_names=straight_names, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
# Keep reserved ⭐ out of Goblin 3-leg so Favorite 3-leg can fill on thin boards.
_top3_ex = straight_names | reserved_fav_names
if len(top3) < 3:
    top3 = pick_top_n(top3_pool, 3, exclude_names=_top3_ex, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
if len(top3) < 3:
    extra = sorted(
    ...
    top3 = pick_top_n(extra, 3, exclude_names=_top3_ex, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
if len(top3) < 3:
    ...
    top3 = pick_top_n(extra_relaxed, 3, exclude_names=_top3_ex, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
# Last resort only: allow dipping into reserved favs if the board cannot fill otherwise.
if len(top3) < 3:
    top3 = pick_top_n(top3_pool, 3, exclude_names=straight_names, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
if len(top3) < 3 and len(rows) <= 30:
    ...
    top3.extend(
        pick_top_n(
            ...
            max_per_game=GAME_CAP,
            max_per_team=TEAM_CAP,
        )
    )
...
for _nm in _fav3_pick_order:
    ...
    if _r["name"] in {x["name"] for x in _fav3_manual}:
        continue
    if _r["name"] in FAVS and not fav3_lane_ok(_r):
        # Thin CBP board: allow loud multi-HR favorites through mild platoon drag.
        if not (_r["hr"] >= 2 and effective_park_pct(_r) >= 20):
            continue
    if _r["name"] not in FAVS and not (_r["hr"] >= 1 or _r["near"] >= 2 or _r["score"] >= 75):
        continue
...
if len(_fav3_manual) == 3:
    fav3 = _fav3_manual
elif len(_early_fav3) == 3:
    fav3 = list(_early_fav3)
elif len(_fav3_manual) >= 1:
    have = {x["name"] for x in fav3}
    for _r in _fav3_manual:
        if _r["name"] in have:
            continue
        fav3.append(_r)
        have.add(_r["name"])
        if len(fav3) == 3:
            break
if len(fav3) < 3 and len(_early_fav3) == 3:
    fav3 = list(_early_fav3)

# Last-resort: remaining ⭐ by attack so the card stays user-⭐ only.
if len(fav3) < 3:
    _loud_fill = sorted(
        [
            r
...
# Final thin-slate lock: 7-prop NYM@PHI board needs explicit non-overlapping 3-leg.
if THIN_SLATE and len(rows) <= 10:
    fav3 = list(_early_fav3) if len(_early_fav3) == 3 else fav3
    fav3_names = {x["name"] for x in fav3}
    forced_top3 = []
    for _nm in ("Brett Baty", "Edmundo Sosa", "Bryson Stott"):
        _r = rows_by_plain.get(_nm)
        if _r and _r["name"] not in straight_names and _r["name"] not in fav3_names:
            forced_top3.append(_r)
    ...
    for _nm in ("Francisco Lindor", "Juan Soto"):
        ...
        if (
            _r
            and _r["name"] not in straight_names
            and _r["name"] not in {x["name"] for x in top3}
        ):
            forced_two.append(_r)
    if len(forced_two) == 2:
        two_leg = forced_two

assert not ({x["name"] for x in two_leg} & straight_names), "2 Leg HR must not reuse Straights of the Day"
assert not ({x["name"] for x in top3} & {x["name"] for x in fav3}), "3 Leg HR and Favorite 3 Leg must be distinct"
...
top5 = pick_top_n(top5_candidates, top5_target, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
...
longshots = pick_top_n(longshot_pool, 4, max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
if len(longshots) < 1:
    thin_long = sorted(
        [
            r
    ...
    longshots = pick_top_n(thin_long, min(4, len(thin_long)), max_per_game=GAME_CAP, max_per_team=TEAM_CAP)
if len(longshots) < 4:
...
assert_game_cap("Top 5 HR Tickets", top5, max_per_game=GAME_CAP)
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "July 16, 2026 — current slate", "href": "index.html"},
        {"date": "2026-07-14", "label": "July 14, 2026 — All-Star Game", "href": "archive/2026-07-14.html"},
        {"date": "2026-07-11", "label": "July 11, 2026", "href": "archive/2026-07-11.html"},
        {"date": "2026-07-10", "label": "July 10, 2026", "href": "archive/2026-07-10.html"},
        {"date": "2026-07-07", "label": "July 7, 2026", "href": "archive/2026-07-07.html"},
        {"date": "2026-07-06", "label": "July 6, 2026", "href": "archive/2026-07-06.html"},
        {"date": "2026-07-03", "label": "July 3, 2026", "href": "archive/2026-07-03.html"},
        {"date": "2026-07-02", "label": "July 2, 2026", "href": "archive/2026-07-02.html"},
        {"date": "2026-07-01", "label": "July 1, 2026", "href": "archive/2026-07-01.html"},
        {"date": "2026-06-29", "label": "June 29, 2026", "href": "archive/2026-06-29.html"},
        {"date": "2026-06-28", "label": "June 28, 2026", "href": "archive/2026-06-28.html"},
        {"date": "2026-06-27", "label": "June 27, 2026", "href": "archive/2026-06-27.html"},
        {"date": "2026-06-26", "label": "June 26, 2026", "href": "archive/2026-06-26.html"},
        {"date": "2026-06-25", "label": "June 25, 2026", "href": "archive/2026-06-25.html"},
        {"date": "2026-06-24", "label": "June 24, 2026", "href": "archive/2026-06-24.html"},
        {"date": "2026-06-23", "label": "June 23, 2026", "href": "archive/2026-06-23.html"},
        {"date": "2026-06-22", "label": "June 22, 2026", "href": "archive/2026-06-22.html"},
        {"date": "2026-06-20", "label": "June 20, 2026", "href": "archive/2026-06-20.html"},
        {"date": "2026-06-19", "label": "June 19, 2026", "href": "archive/2026-06-19.html"},
        {"date": "2026-06-18", "label": "June 18, 2026", "href": "archive/2026-06-18.html"},
        {"date": "2026-06-17", "label": "June 17, 2026", "href": "archive/2026-06-17.html"},
        {"date": "2026-06-16", "label": "June 16, 2026", "href": "archive/2026-06-16.html"},
        {"date": "2026-06-15", "label": "June 15, 2026", "href": "archive/2026-06-15.html"},
        ...
        {"date": "2026-06-08", "label": "June 8, 2026", "href": "archive/2026-06-08.html"},
        {"date": "2026-06-06", "label": "June 6, 2026", "href": "archive/2026-06-06.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Thursday, July 16, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )

    games_word = "game" if TOTAL_GAMES == 1 else "games"
    count_sentence = (
        f"This board covers <strong>{TOTAL_ROWS} listed HR props</strong> across "
        f"<strong>{TOTAL_GAMES} {games_word}</strong>, with <strong>{TOTAL_FAVS} Worst Pickz Favorite</strong> rows (&#11088;)."
    )
