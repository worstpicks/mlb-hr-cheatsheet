#!/usr/bin/env python3
"""Synthesize the hr-matchups game PropFinder did not export for 2026-08-27.

  STL  Justin Bruihl -> Cooper Hjerpe   MLB still lists St. Louis as TBD; the sheet's
                          owner named Hjerpe and exported Bruihl as a pitch-mix
                          stand-in. Batter rows come from the Bruihl export -- same
                          Baltimore lineup, and batter form does not depend on who is
                          pitching. Bruihl's pitcher block is NOT carried: those are
                          Bruihl's measured numbers and would read as Hjerpe's.
                          Hjerpe has never thrown a major-league inning, so his header
                          states the debut rather than a fabricated split.
"""
...
GAPS: dict[str, dict] = {
    "Cooper Hjerpe": {
        ...
        "oteam": "BAL",
        "lineups": "sibling",
        "sibling": f"hr-matchups-BAL-at-STL-Justin-Bruihl-{DATE}.csv",
        "block_from": None,
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-BAL-at-STL-Justin-Bruihl-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 8/27 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# Nothing is carried. Cooper Hjerpe is making his major-league debut, so there is no
# prior PropFinder score anywhere and no measured lane to fall back on. Bruihl's row is
# Bruihl's -- putting it under Hjerpe's name would be a fabrication, not a stand-in.
