#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-08-29.

Three whole games shipped weak-spots, zone rows and HR risk but no matchup file at
all -- KC @ CLE, SD @ TB and COL @ ATL. All six arms are rebuilt from the zone
lineups plus their own pitcher-summary splits.

  PIT  Carmen Mlodzinski -> Lake Bachar   MLB replaced the probable after the export.
                          Bachar inherits the Mlodzinski file's St. Louis batter rows
                          -- batter form does not depend on who is pitching -- but not
                          Mlodzinski's pitcher block. Bachar has a real 2026 book
                          (73.0 IP, .194 BAA), so his header renders measured rates.
"""
...
GAPS: dict[str, dict] = {
    "Foster Griffin":   {"matchup": "KC @ CLE",  "pteam": "CLE", "oteam": "KC",  "lineups": "zone"},
    "Daniel Lynch IV":  {"matchup": "KC @ CLE",  "pteam": "KC",  "oteam": "CLE", "lineups": "zone"},
    "Nick Martinez":    {"matchup": "SD @ TB",   "pteam": "TB",  "oteam": "SD",  "lineups": "zone"},
    "Walker Buehler":   {"matchup": "SD @ TB",   "pteam": "SD",  "oteam": "TB",  "lineups": "zone"},
    "Martin Perez":     {"matchup": "COL @ ATL", "pteam": "ATL", "oteam": "COL", "lineups": "zone"},
    "Ryan Feltner":     {"matchup": "COL @ ATL", "pteam": "COL", "oteam": "ATL", "lineups": "zone"},
    "Lake Bachar": {
        "matchup": "PIT @ STL",
        "pteam": "PIT",
        "oteam": "STL",
        "lineups": "sibling",
        "sibling": f"hr-matchups-PIT-at-STL-Carmen-Mlodzinski-{DATE}.csv",
        "block_from": None,
    },
}

SUPERSEDED: list[str] = [
    f"hr-matchups-PIT-at-STL-Carmen-Mlodzinski-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 8/29 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# Nothing is carried. The six zone-rebuilt arms all have their own same-day HR-risk
# rows, and Lake Bachar has none anywhere -- Mlodzinski's row would be Mlodzinski's.
