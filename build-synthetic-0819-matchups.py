#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-08-19.

Three arms the exports miss, all confirmed against the MLB Stats API:

  CWS  Jose Urquidy -> Sean Newcomb   (PropFinder updated weak-spots and HR risk
                                       but not the matchup export)
  SF   Adrian Houser -> Matt Wilkinson
  HOU  no export at all for Ethan Pecko

Newcomb is rebuilt from that slate's zone lineups plus his real pitcher-summary
splits. Wilkinson takes the Houser export's batter rows -- same Cleveland lineup,
same day, and batter form is pitcher-independent. Pecko is the awkward one: no
2026-08-19 file carries Anaheim's batters at all, so his lineup comes from the most
recent Angels export (2026-08-13). That form is six days old and is the best
available; it affects exactly one listed prop.
"""
...
GAPS: dict[str, dict] = {
    ...
    # Houser is the SF arm in the stale export, so his batter rows are the CLE lineup.
    "Matt Wilkinson": {
        "matchup": "SF @ CLE",
        "pteam": "SF",
        "oteam": "CLE",
        "lineups": "sibling",
        "sibling": f"hr-matchups-SF-at-CLE-Adrian-Houser-{DATE}.csv",
        "block_from": None,
    },
    # No 8/19 export carries Anaheim's batters; fall back to the most recent Angels
    # export. Six-day-old form, flagged, and it drives a single listed prop.
    "Ethan Pecko": {
        "matchup": "LAA @ HOU",
        "pteam": "HOU",
        "oteam": "LAA",
        "lineups": "sibling",
        "sibling": "hr-matchups-TEX-at-LAA-Jacob-deGrom-2026-08-13.csv",
        "block_from": None,
    },
}
...
SUPERSEDED: list[str] = [
    f"hr-matchups-CWS-at-CHC-Jos-Urquidy-{DATE}.csv",
    f"hr-matchups-SF-at-CLE-Adrian-Houser-{DATE}.csv",
]
...
# PropFinder's 8/19 export already scores Newcomb; the other two gap arms have
# no measured history anywhere, so nothing is carried.
