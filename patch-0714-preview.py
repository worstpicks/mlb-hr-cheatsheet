#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-14 All-Star Game. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-11.html"
SHEET_DATE = "2026-07-14"

spec = importlib.util.spec_from_file_location("build0714", ROOT / "build-sheet-2026-07-14.py")
...
# All-Star Game = one board; allow stacking beyond the usual 2/game diversification cap.
ASG_SINGLE = TOTAL_GAMES <= 1
GAME_CAP = 5 if ASG_SINGLE else 2
TEAM_CAP = None if ASG_SINGLE else 2
# Same-game O0.5 / O1.5 is required when there is only one game.
STRAIGHT_EXCLUDE_GAME = False if ASG_SINGLE else True
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "July 14, 2026 — All-Star Game (current)", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Tuesday, July 14, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
