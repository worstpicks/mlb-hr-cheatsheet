#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-06. Does not commit or push."""
...
SHEET_DATE = "2026-08-06"

spec = importlib.util.spec_from_file_location("build0806", ROOT / "build-sheet-2026-08-06.py")
...
# 8/6 judgment: O0.5 Hill 💎 vs Mikolas 🧤 (+1.57 / 1.64, CBP +40, 2 HR / 2 near).
# O1.5 Abreu ⭐ vs Castillo (3 HR / 3 near) — different game.
_hill = next((r for r in straight_rows if r["name_plain"] == "Derek Hill"), None)
_abreu = next((r for r in straight_rows if r["name_plain"] == "Wilyer Abreu"), None)
if _hill is not None:
    straight_o05 = _hill
if _abreu is not None and _abreu["game_key"] != straight_o05["game_key"]:
    straight_o15 = _abreu
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/6 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Bryce Harper", "Josh Bell", "Pete Alonso"]
_two_lock_names = ["Coby Mayo", "Brice Turang"]
_fav3_lock_names = ["Gunnar Henderson", "J.T. Realmuto", "Eugenio Suarez"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 6, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-05",
        "2026-08-04",
