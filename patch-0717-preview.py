#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-17. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-16.html"
SHEET_DATE = "2026-07-17"

spec = importlib.util.spec_from_file_location("build0717", ROOT / "build-sheet-2026-07-17.py")
...
# 7/17 Favorite 3-leg: attack-ranked user ⭐ (algorithmic pool; optional name nudge).
_fav3_pick_order = [
    "Ben Rice",
    "Junior Caminero",
    "Rafael Devers",
...
if len(_forced_fav3) == 3:
    ...
    # Rebuild Goblin 3-leg without Fav3 / straights.
    ...
    # Rebuild 2-leg without Fav3 / 3-leg / straights.
    _two_pool = sorted(
        [
            r
            for r in straight_rows
    ...
    two_leg = pick_top_n(
        _two_pool,
        2,
        exclude_names=straight_names | fav3_names | {x["name"] for x in top3},
        max_per_game=1,
        max_per_team=2,
    )
...
def update_manifest():
    ...
    if "2026-07-16" in old:
        old["2026-07-16"] = {
            "date": "2026-07-16",
            "label": "July 16, 2026",
            "href": "archive/2026-07-16.html",
        }
    ordered = [
        {"date": SHEET_DATE, "label": "July 17, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-07-16",
        "2026-07-14",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Friday, July 17, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
