#!/usr/bin/env python3
"""Build 2026-07-22 sheet from imported CSVs and user prop list."""
...
DATE = "2026-07-22"
OUT = ROOT / "build-sheet-2026-07-22.py"
BUM_RISK_MIN = 0.95
...
RAW_PROPS = [
    ...
    "Ryan Vilade💎",
    "Hunter Feduccia",
    # Night BAL@BOS (Bradish/Suarez) has no PropFinder matchups yet —
    # day-game Willson/Wilyer already listed above vs Kremer/Bennett.
    "Matt Olson",
    "Drake Baldwin",
    "Mike Yastrzemski⭐",
    "Jackson Merrill",
...
CHC_DH_SPECS = {
    "PIT @ NYY (G1)": {
        "away_sp": "Keller",
        "home_sp": "Cole",
        "away_sp_full": "Mitch Keller",
        "home_sp_full": "Gerrit Cole",
        "files": [
            "hr-matchups-PIT-at-NYY-Mitch-Keller-2026-07-22.csv",
            "hr-matchups-PIT-at-NYY-Gerrit-Cole-2026-07-22.csv",
        ],
    },
    "PIT @ NYY (G2)": {
        "away_sp": "Chandler",
        "home_sp": "Fried",
        "away_sp_full": "Bubba Chandler",
        "home_sp_full": "Max Fried",
        "files": [
            "hr-matchups-PIT-at-NYY-Bubba-Chandler-2026-07-22.csv",
            "hr-matchups-PIT-at-NYY-Max-Fried-2026-07-22.csv",
        ],
    },
}
...
def split_chc_doubleheader(games: dict[str, dict]) -> None:
    if "PIT @ NYY" not in games:
        return
    games.pop("PIT @ NYY", None)
...
def parse_prop_label(raw: str) -> tuple[str, bool, bool, str | None]:
    ...
    if m:
        game_override = f"PIT @ NYY (G{m.group(1)})"
