#!/usr/bin/env python3
"""Build 2026-09-07 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-07"
OUT = ROOT / "build-sheet-2026-09-07.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0907-matchups.py. Nothing is carried today: every arm the sheet
...
# Two Max Muncys are exported again: Oakland's right-handed one (in their
# bench-inclusive export, not in the posted lineup) and the Dodgers' left-handed
# third baseman batting fourth. The owner's list places him among Dodgers bats and
# the listed hand there is L, so pin the prop to that game rather than letting
# batter_ctx resolve it by whichever game sorted last.
...
PROBABLE_OVERRIDES: dict[str, dict] = {
    # The owner exported Bobby Miller for the Dodgers because his pitch mix is close
    # to the projected starter's; the arm actually taking the mound is Emmet Sheehan.
    # Only the NAME moves. Sheehan has no PropFinder risk row and no export of his
    # own, so his header falls through to his real MLB season figure rather than
    # printing Miller's measured lanes under Sheehan's name.
    "CIN @ LAD": {
        "side": "home",
        "from": "Miller",
        "to": "Sheehan",
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-07 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-07 sent "Alejanro Kirk", which matched nothing at
