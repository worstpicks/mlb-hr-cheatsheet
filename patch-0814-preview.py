#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-14. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-08-13.html"
...
# 8/14 judgment: O0.5 Teoscar Hernandez vs Gasser — the top-ranked lane with nothing
# working against it: zone fit 35.1 carrying the board's best zone hard-hit (65.1), 1 HR
# and 3 near-HR at 94 EV and heating, +0.67 split as a RHB against a LHP, and +12% park
# (+9% RHB) at Dodger Stadium. Caissie outranks him on raw attack and owns the slate's
# best zone fit (38.9), barrel zone (48.1) and park (+20%), but Chase Burns is the wrong
# arm to attack at -0.40 risk on a 2.61 ERA and the +0.01 split is flat — the rubric wants
# an attackable pitcher, so Caissie anchors the 3-leg instead. Reject Yordan Alvarez and
# Jac Caglianone despite top-10 attack ranks: 0 HR in the window is no form to ride.
# O1.5 Jackson Merrill vs Gavin Williams 🧤 — the slate's only genuine multi-HR profile:
# 3 HR and 4 near-HR at 98 EV, heating, high blast, on a +0.93 split into a 1.12-risk glove
# bum leaking 1.38 HR/9. Caissie ranks higher on the multi board but carries 2 HR against a
# pitcher who suppresses them; for an over-1.5 the loud repeat power plus the bum lane is
# the better bet. Different game from the O0.5 and a different pair than 8/13 shipped.
_o05_lock = next((r for r in straight_rows if r["name_plain"] == "Teoscar Hernandez"), None)
_o15_lock = next((r for r in straight_rows if r["name_plain"] == "Jackson Merrill"), None)
...
# 8/14 Goblin judgment: distinct lanes after the straights; reserve the ⭐ bats for Fav3.
# 3 Leg — Owen Caissie (top non-favorite attack lane on the slate: best zone fit 38.9 and
# barrel zone 48.1, 2 HR/2 near at 94 EV heating with high blast, and the slate's biggest
# park at +20% in Cincinnati), Jesus Sanchez (zone 37.8 with a 49.1 barrel zone, +0.30 split
# and +0.48 risk into Gerrit Cole), Mookie Betts (fully positive lane: +0.67 split, +0.68
# risk, +12% park, 1 HR/2 near at 93 EV heating). Three different games.
# 2 Leg — Zack Gelof (+0.49 split into Kumar Rocker 🧤 at 1.06 risk and 1.30 HR/9, plus the
# slate's biggest park at +32% game / +13% RHB, 1 HR/2 near) and Victor Mesa Jr. (2 HR/2
# near at 96 EV heating with high blast on a 37.0 zone fit and a +0.56 split).
# Fav 3 — Pete Alonso (the board's best zone-HR rate at 40.8 with a +1.01 split as a RHB
# vs a LHP, +0.31 risk, +9% RHB park), Lawrence Butler (3 HR/3 near at 99 EV heating, +1.22
# split into Rocker 🧤, +32% park) and Munetaka Murakami (2 HR/2 near at 98 EV heating, zone
# 33.3 with a 27.3 zone-HR and 37.2 barrel zone, +0.44 split). Reject Mike Trout and Luis
# Robert despite top-five favorite attack ranks: both sit on negative splits against arms
# that suppress HR. Reject Abimelec Ortiz despite 3 HR/3 near and a +1.28 split — a -10%
# park both ways is too much drag for a single-leg favorite.
_top3_lock_names = ["Owen Caissie", "Jesus Sanchez", "Mookie Betts"]
_two_lock_names = ["Zack Gelof", "Victor Mesa Jr."]
_fav3_lock_names = ["Pete Alonso", "Lawrence Butler", "Munetaka Murakami"]
...
def update_manifest():
    ...
    if "2026-08-13" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-08-13"] = {
            "date": "2026-08-13",
            "label": "August 13, 2026",
            "href": "archive/2026-08-13.html",
        }
    if "2026-08-09" in old or (ROOT / "preview" / "archive" / "2026-08-09.html").is_file():
        old["2026-08-09"] = {
            "date": "2026-08-09",
            "label": "August 9, 2026",
            "href": "archive/2026-08-09.html",
        }
    if "2026-08-08" in old or (ROOT / "preview" / "archive" / "2026-08-08.html").is_file():
        old["2026-08-08"] = {
            "date": "2026-08-08",
            "label": "August 8, 2026",
            "href": "archive/2026-08-08.html",
        }
    if "2026-08-07" in old or (ROOT / "preview" / "archive" / "2026-08-07.html").is_file():
        old["2026-08-07"] = {
            "date": "2026-08-07",
            "label": "August 7, 2026",
            "href": "archive/2026-08-07.html",
        }
    if "2026-08-06" in old or (ROOT / "preview" / "archive" / "2026-08-06.html").is_file():
        old["2026-08-06"] = {
            "date": "2026-08-06",
            "label": "August 6, 2026",
            "href": "archive/2026-08-06.html",
        }
    if "2026-08-05" in old or (ROOT / "preview" / "archive" / "2026-08-05.html").is_file():
        old["2026-08-05"] = {
            "date": "2026-08-05",
            "label": "August 5, 2026",
            "href": "archive/2026-08-05.html",
        }
    if "2026-07-31" not in old:
        old["2026-07-31"] = {
            "date": "2026-07-31",
            "label": "July 31, 2026",
            "href": "archive/2026-07-31.html",
        }
    if "2026-07-30" not in old:
        old["2026-07-30"] = {
            "date": "2026-07-30",
            "label": "July 30, 2026",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 14, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-13",
        "2026-08-12",
        ...
        elif date == "2026-08-01":
            ordered.append(
                {
                    "date": "2026-08-01",
                    "label": "August 1, 2026",
                    "href": "archive/2026-08-01.html",
                }
            )
        elif date == "2026-07-31":
            ordered.append(
                {
                    "date": "2026-07-31",
                    "label": "July 31, 2026",
    ...
    # The hand-maintained date list above has drifted from disk over time, leaving
    # published archives unreachable from the picker. Backfill anything on disk that
    # the list missed so every archive stays selectable.
    listed = {entry["date"] for entry in ordered}
    archive_dir = ROOT / "preview" / "archive"
    extra = sorted(
        (p.stem for p in archive_dir.glob("20??-??-??.html") if p.stem not in listed),
        reverse=True,
    )
    for date in extra:
        y, m, d = date.split("-")
        ...
        ordered.append(
            {
                "date": date,
                "label": f"{month} {int(d)}, {y}",
                "href": f"archive/{date}.html",
            }
        )
    if extra:
        print(f"manifest backfilled {len(extra)} archive(s): {', '.join(extra)}")
    ordered.sort(key=lambda e: (e["date"] != SHEET_DATE, [-int(x) for x in e["date"].split("-")]))
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Friday, August 14, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
