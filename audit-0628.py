#!/usr/bin/env python3
"""Quick audit for 2026-06-28 sheet before push."""
...
if 'sheet-date" content="2026-06-28"' not in html:
...
if zc < EXPECTED_PROPS:
    errors.append(f"zoneScore count {zc} < {EXPECTED_PROPS} props")

for needle in ("Homerun Form", "Split + Risk + Park + Form + Zone", "Worst Pickz Straights"):
    if needle not in html:
        errors.append(f"missing feature: {needle}")

if not (ROOT / "preview" / "data" / "research-2026-06-28.json").exists():
    errors.append("missing preview/data/research-2026-06-28.json")
if not (ROOT / "preview" / "data" / "park-factors-2026-06-28.json").exists():
    errors.append("missing preview/data/park-factors-2026-06-28.json")
...
spec = importlib.util.spec_from_file_location("b", ROOT / "build-sheet-2026-06-28.py")
...
if manifest["sheets"][0]["date"] != "2026-06-28":
    errors.append("manifest current slate not 2026-06-28")
if not (ROOT / "preview" / "archive" / "2026-06-27.html").exists():
    errors.append("missing archive 2026-06-27.html")

if errors:
    print("FAIL audit-0628:")
    for e in errors:
        print(" ", e)
    raise SystemExit(1)

print(
    f"OK audit-0628 — sheet-date, {EXPECTED_GAMES}/{EXPECTED_GAMES} park headers, "
