#!/usr/bin/env python3
"""Fill hr-matchups PropFinder did not export for 2026-09-17.

  MIN @ LAA shipped weak-spots for both arms but no hr-matchups file for either, and
  none is in Downloads. Today's zone-matchups export carries both real lineups (14
  rows each) with ZoneScore, and both arms are in pitcher-summary and HR risk, so the
  game is rebuilt from those plus each hitter's most recent real batted-ball form.
  The one thing no other export carries is ODDS, which stay N/A until the matchup file
  is re-exported -- at which point a real export here is used instead of this.
"""
...
GAPS: dict[str, dict] = {
    # Nothing to fill: MIN @ LAA was gap-filled from zone lineups until the owner
    # re-exported it, and the real files (with odds) replaced the synthetic ones.
}
