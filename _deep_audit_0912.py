#!/usr/bin/env python3
"""Deep content audit for the 2026-09-12 sheet: prop coverage, markers, bums, staleness."""
...
# A name on the owner's list who is on nobody's roster today. Ty France is a free
# agent -- no current team, no game on this slate -- so there is no matchup to score
# him in and no honest row to print. Naming him here keeps the audit strict: every
# OTHER listed prop must still appear, and if he signs somewhere and starts, this
# entry stops matching and the check goes back to demanding a row.
NOT_ON_SLATE: dict[str, str] = {
    "Ty France": "free agent - no current team, not on any 2026-09-12 roster",
}

# (name, game override, marker), generated from build-0912-from-csv.py RAW_PROPS so
...
EXPECTED_BUMS = {'Ian Seymour', 'Zach Thornton', 'Brady Singer'}
...
def main() -> int:
    ...
    # 2026-09-12 rebalance the model ended in a clamp at 58 and a third of every
