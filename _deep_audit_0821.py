#!/usr/bin/env python3
"""Deep content audit for the 2026-08-21 sheet: prop coverage, markers, bums, staleness."""
...
USER_PROPS: list[tuple[str, str | None, str]] = [
    ...
    ('JT Realmuto', ANY, ""),
    ('Joshua Baez', ANY, ""),
    ('Ivan Herrera', ANY, STAR),
    ('Amed Rosario', ANY, GEM),
    ('Heliot Ramos', ANY, ""),
    ('Ben Rice', ANY, ""),
    ('Jesus Sanchez', ANY, GEM),
    ('Alejandro Kirk', ANY, GEM),
    ('Charles McAdoo', ANY, ""),
    ...
    ('Yandy Diaz', ANY, STAR),
    ('Ryan Vilade', ANY, STAR),
    ('Munetaka Murakami', ANY, STAR),
    ('Miguel Vargas', ANY, ""),
    ('Luisangel Acuna', ANY, ""),
    ('Randal Grichuk', ANY, ""),
    ('Brett Baty', ANY, STAR),
    ('Francisco Lindor', ANY, STAR),
    ('AJ Ewing', ANY, ""),
    ('Carson Benge', ANY, ""),
    ('Michael Massey', ANY, ""),
...
EXPECTED_BUMS = {"Tanner Gordon", "Connor Prielipp", "Sean Manaea", "Ryan Gusto"}

# Every game carries two pitcher segments. Randy Vasquez has no PropFinder HR-risk
# row -- the export predates his being named -- so his header renders his real
# measured BAA/HR-9 lane from the carried pitcher block instead of a risk score.
...
SUPERSEDED_PROBABLES: list[str] = ["Joe Musgrove", "Will Dion"]
...
def main() -> int:
    ...
    for name in STALE_PREV_DAY:
        if fold(name) in body_folded:
            fail(f"stale 8/20 content on sheet: {name}")
