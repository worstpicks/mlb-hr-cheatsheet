#!/usr/bin/env python3
"""Hard verify June 11 sheet: duplicates, counts, summary, straights, bums."""
...
if sheet_date != "2026-06-11":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Thursday, June 11, 2026" not in text:
    errors.append("header weekday wrong: expected Thursday, June 11, 2026")
...
stale_june10 = ["Brandon Marsh", "Brent Rooker", "Junior Caminero", "Nick Kurtz", "Max Scherzer"]
for stale in stale_june10:
    if stale in text:
        errors.append(f"stale June 10 name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0611:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0611")
