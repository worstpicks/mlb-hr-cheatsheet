#!/usr/bin/env python3
"""Deep content audit for the 2026-09-05 sheet: prop coverage, markers, bums, staleness."""
...
GAMECOUNT = 13
WEEKDAY = "Saturday"
DATE_TEXT = "September 5, 2026"

# (name, game override, marker), generated from build-0905-from-csv.py RAW_PROPS so
# the audit cannot drift from what was actually built. Two Max Muncys again today
# (Oakland's RHB, who is not in their posted lineup, and the Dodgers' LHB batting
# fourth). The listed prop is the Dodgers one -- it sits inside the WSH @ LAD run
# of the owner's list and the listed hand is L -- so it carries its game.
ANY = None
...
RESOLVED: dict[str, str] = {
    # The owner's list typed "Alejandro kirk"; the build adopts the export's
    # capitalisation rather than shipping the typo, so the sheet says Kirk.
}
...
EXPECTED_BUMS = {'Ethan Pecko', 'Jeffrey Springs', 'Mason Adams', 'Matthew Liberatore'}
...
SUPERSEDED_PROBABLES: list[str] = []  # every exported arm is today's MLB probable
