#!/usr/bin/env python3
"""Build 2026-08-29 sheet from imported CSVs and user prop list."""
...
DATE = "2026-08-29"
OUT = ROOT / "build-sheet-2026-08-29.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0829-matchups.py. Nothing is carried today: every arm the sheet
...
ALIASES = {
    # no prop-list typos on this slate
...
DOUBLEHEADER_SPECS: dict[str, dict[str, dict]] = {
    "BOS @ NYY": {
        "BOS @ NYY (G1)": {
            "away_sp_full": "Jake Bennett",
            "home_sp_full": "Carlos Rodon",
            "files": [
                f"hr-matchups-BOS-at-NYY-Jake-Bennett-{DATE}.csv",
                f"hr-matchups-BOS-at-NYY-Carlos-Rodon-{DATE}.csv",
            ],
        },
        "BOS @ NYY (G2)": {
            "away_sp_full": "Brayan Bello",
            "home_sp_full": "Max Fried",
            "files": [
                f"hr-matchups-BOS-at-NYY-Brayan-Bello-{DATE}.csv",
                f"hr-matchups-BOS-at-NYY-Max-Fried-{DATE}.csv",
            ],
        },
    },
}
