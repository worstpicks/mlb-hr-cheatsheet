#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-08-21.

Three arms, all confirmed against the MLB Stats API:

  CLE  Joey Cantillo   -> no hr-matchups export for either side of CLE @ COL,
  COL  Tanner Gordon      even though weak-spots, zone-matchups, pitcher-summary
                          and HR risk all shipped. Rebuilt from the zone lineups
                          plus each arm's real pitcher-summary splits.

  SD   Joe Musgrove -> Randy Vasquez   The probable changed after the export.
                          Vasquez inherits the Musgrove file's batter rows -- those
                          are today's real Minnesota bats and batter form is
                          pitcher-independent -- and his own Season/vsLHB/vsRHB
                          block is carried from his last real export (2026-08-15).

ATL @ MIL is deliberately absent: it was already in progress when the slate was
pulled, PropFinder exported no props or matchups for it, and neither arm is a bum.
"""
...
GAPS: dict[str, dict] = {
    ...
    # Musgrove was scratched in favour of Vasquez; his export carries the Minnesota
    # lineup Vasquez actually faces, so the batter rows are today's real numbers.
    "Randy Vasquez": {
        "matchup": "MIN @ SD",
        "pteam": "SD",
        "oteam": "MIN",
        "lineups": "sibling",
        "sibling": f"hr-matchups-MIN-at-SD-Joe-Musgrove-{DATE}.csv",
        "block_from": "hr-matchups-SD-at-CLE-Randy-Vasquez-2026-08-15.csv",
    },
}

# Stale-probable exports: kept on disk as the lineup source above, dropped from the
# manifest so derive_games_from_csv does not see two arms on one side.
SUPERSEDED: list[str] = [
    f"hr-matchups-MIN-at-SD-Joe-Musgrove-{DATE}.csv",
]
...
def batter_row(zrow: dict, prior_index: dict[str, dict]) -> tuple[list[str], bool]:
    ...
    row = [
        f"{name} {hand}",
        "—",
        "N/A",  # no 8/21 price exported for this game
        zone,
        p("L5 PA/G"),
        p("BBE"),
        p("HR", "0"),
...
# PropFinder's 8/21 hr-targets-overall export covers 30 of the slate's 30 listed
# arms -- but it was pulled while Joe Musgrove was still San Diego's probable, so
# Randy Vasquez has no risk row anywhere and Musgrove's own cells are "-".
#
# Nothing is carried. There is no prior PropFinder risk score for Vasquez to carry
# forward, and inventing one would put a number on the sheet that is not comparable
# to the scale every other arm is graded on. The build renders his real measured
# BAA/HR-9 lane from the carried pitcher block instead.
