#!/usr/bin/env python3
"""Rank 2026-08-18 hitters by grand-slam upside.
...
def sheet_rows() -> dict[tuple[str, str], dict]:
    ...
    src = (ROOT / "build-sheet-2026-08-18.py").read_text(encoding="utf-8")
...
def main() -> None:
    ...
    for g in sorted(research["games"], key=lambda x: x.get("startTime") or ""):
        ...
        for side, opp_key in (("awayLineup", "homePitcher"), ("homeLineup", "awayPitcher")):
            ...
            for r in lineup:
                order = r["order"]
                # Slots 10+ are bench/depth, not the batting order. Verified against
                # the MLB boxscore each slate -- a hitter parked at 10+ is genuinely
                # out, not merely unranked.
                if order > 9:
                    continue
