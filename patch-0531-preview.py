#!/usr/bin/env python3
"""Patch preview sheet to 2026-05-31. Does not commit or push."""
...
ARCHIVE_0530 = ROOT / "preview" / "archive" / "2026-05-30.html"
GAMES_BLOCK = (ROOT / "_games-0531.txt").read_text(encoding="utf-8-sig").strip()

SHEET_DATE = "2026-05-31"

spec = importlib.util.spec_from_file_location("build0531", ROOT / "build-sheet-2026-05-31.py")
...
longshots = [r for r in top_with_listed if (r["odds_value"] or 0) >= 700][:4]
if len(longshots) < 4:
    extra = [r for r in top_with_listed if r not in longshots]
...
STRAIGHT_OF_DAY_CARD = f"""                <div class="summary-card full-width straight-of-day-card">
                    ...
                    <p class="model-note summary-note">Top current-slate HR lanes built from score, odds, pitcher splits, and park context.</p>
                    <div class="straight-picks-grid">
                        <div class="straight-pick-hero">
                            ...
                            <ul class="straight-pick-factors">
                                <li><strong>Form</strong><small>{straight_o05['note']}</small></li>
                            </ul>
                        ...
                        <div class="straight-pick-hero straight-pick-hero--o15">
                            ...
                            <ul class="straight-pick-factors">
                                <li><strong>Multi-HR profile</strong><small>{straight_o15['note']}</small></li>
                            </ul>
                            <div class="straight-pick-actions">
...
GOBLIN_CARD = f"""                <div class="summary-card full-width best-bets-card">
                    ...
                    <p class="model-note summary-note">Auto-ranked from fresh 05-31 CSVs: hitter form, pitcher split weakness, and park/weather influence.</p>
                    <div class="best-bets-grid">
                        ...
                        <div class="best-bets-group">
                            <h4>Hits Parlay</h4>
                            <ul>
                                <li><strong>{", ".join(r['name_plain'] for r in top5[:3])}</strong><small>Top score lane core.</small></li>
                                <li><strong>{", ".join(r['name_plain'] for r in top5[3:5])}</strong><small>High-upside additions by score and matchup.</small></li>
                            </ul>
...
TOP_CARD = """                <div class="summary-card full-width top-five-card">
                    ...
                    <p class="model-note summary-note">Ranked from fresh 05-31 CSV imports.</p>
                    <div class="top-five-list">
""" + "\n".join(
    f'                        <div class="top-five-item"><span>{r["name_plain"]} <small>vs {r["chip"]} • {r["game_key"]}</small></span><strong>{r["score"]}</strong></div>'
    for r in top5
) + """
                    </div>
                </div>"""
...
FADES_INNER = "\n".join(
    f'                        <div class="summary-item"><span>{w["game"]} <small>{w["venue"]} downside lane</small></span><strong>{w["hr_pct_text"]}</strong></div>'
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "May 31, 2026 — current slate", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Sunday, May 31, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
...
def main():
    ARCHIVE_0530.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(PREVIEW, ARCHIVE_0530)
    print("archived current preview to", ARCHIVE_0530.relative_to(ROOT))
