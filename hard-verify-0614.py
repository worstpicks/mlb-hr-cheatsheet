...
if sheet_date != "2026-06-14":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Sunday, June 14, 2026" not in text:
    errors.append("header weekday wrong: expected Sunday, June 14, 2026")
...
stale_june13 = [
    "Hunter Goodman", "Willi Castro", "Cole Carrigg", "Colby Thomas", "Jake Bauers",
    "Shea Langeliers", "Henry Bolte", "TJ Rumfield", "Jimmy Crooks", "Ben Rice",
    "Rhys Hoskins", "Adley Rutschman", "Fernando Tatis Jr.", "Jordan Walker",
    "Nelson Velazquez", "Ryan McMahon", "Cody Bellinger", "Dylan Crews",
    "Endy Rodriguez", "Ryan O'Hearn", "Kyle Stowers", "Owen Caissie", "Blaze Alexander",
    "Jorge Mateo", "Angel Martinez", "Spencer Torkelson", "Wenceel Perez", "Gleyber Torres",
    "Tyler Stephenson", "Gabriel Moreno", "Ryan Ward", "Freddie Freeman", "Dalton Rushing",
    "Jarren Duran", "Caleb Durbin", "Gary Sanchez", "Jackson Chourio", "Garrett Mitchell",
    "Kyle Schwarber", "Jeremy Pena", "Jonah Cox", "Logan O'Hoppe", "Mike Burrows",
    "Sean Sullivan", "Kyle Freeland", "Joey Estes",
]
for stale in stale_june13:
    if stale in text:
        errors.append(f"stale prior-slate name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0614:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0614")
