#!/usr/bin/env python3
"""Final local audit for 2026-08-01 (do not push)."""
...
def main() -> int:
    ...
    if "August 1, 2026 — current slate" not in html:
        fail("manifest missing August 1 current slate")
    ...
    if not arch.exists():
        fail("7/31 archive missing")
    if 'content="2026-07-31"' not in arch.read_text(encoding="utf-8"):
        fail("7/31 archive wrong sheet-date")
