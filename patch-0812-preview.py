#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-12. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-08-11.html"
SHEET_DATE = "2026-08-12"

spec = importlib.util.spec_from_file_location("build0812", ROOT / "build-sheet-2026-08-12.py")
...
# 8/12 judgment: O0.5 De La Cruz vs Castillo 🧤 — +1.31 split into a 1.04-risk bum with
# the slate's best LHB park (+15% at Rate Field, wind out), 2 HR/2 near-HR at 92.7 EV.
# Reject Mesa Jr. despite the bigger split: his hand park is only +1% and 84.7 EV is soft.
# Reject Schwarber despite the top zone score: Leahy is neutral (+0.03) and Busch is -4% LHB.
# O1.5 Alonso vs Matthews — best true multi profile on the board (2 HR, 3 near-HR, 95.3 EV)
# on a +0.85 split against a 5.23-ERA arm; only a -3% park nudge against it.
_delacruz = next((r for r in straight_rows if r["name_plain"] == "Elly De La Cruz"), None)
_alonso = next((r for r in straight_rows if r["name_plain"] == "Pete Alonso"), None)
if _delacruz is not None:
    straight_o05 = _delacruz
if _alonso is not None and _alonso["game_key"] != straight_o05["game_key"]:
    straight_o15 = _alonso
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/12 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
# 3 Leg — DeLuca (top score 97, 3 HR/3 near vs Perkins 🧤, +10% RHB park), Caminero (same
# bum lane, 94.0 EV, heating), Massey (slate-best split +1.09 into a 0.91-risk arm).
# 2 Leg — Diaz (+0.46 into Perkins 🧤, +10% park) and Suarez (best zone fit vs Castillo 🧤).
# Fav 3 — the three best ⭐ lanes left after Alonso goes to the O1.5 straight: Schwarber
# (slate-best zone fit + attack rank), Murakami (98.2 EV into a +15% LHB park),
# Raleigh (+12% park, 3 near-HR, 95.8 EV). Carroll rejected everywhere: -18% LHB park.
_top3_lock_names = ["Jonny DeLuca", "Junior Caminero", "Michael Massey"]
_two_lock_names = ["Yandy Diaz", "Eugenio Suarez"]
_fav3_lock_names = ["Kyle Schwarber", "Munetaka Murakami", "Cal Raleigh"]
...
def update_manifest():
    ...
    if "2026-08-11" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-08-11"] = {
            "date": "2026-08-11",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 12, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-11",
        "2026-08-10",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Wednesday, August 12, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
