#!/usr/bin/env python3
"""Hard verify June 10 sheet: duplicates, counts, summary, straights, bums."""
...
if sheet_date != "2026-06-10":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Wednesday, June 10, 2026" not in text:
    errors.append("header weekday wrong: expected Wednesday, June 10, 2026")
...
stale_june9 = ["Shea Langeliers", "Colby Thomas", "Eric Lauer", "Robert Gasser", "Chase Burns"]
for stale in stale_june9:
    if stale in text:
        errors.append(f"stale June 9 name still in preview: {stale}")

if errors:
    print("FAIL hard-verify-0610:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0610")
