#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-18. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-17.html"
SHEET_DATE = "2026-07-18"

spec = importlib.util.spec_from_file_location("build0718", ROOT / "build-sheet-2026-07-18.py")
...
# 7/18 Favorite 3-leg: user ⭐ only (Littell 🧤 / Sheehan attack lanes first).
_fav3_pick_order = [
    "Tyler Soderstrom",
    "James Wood",
    "CJ Abrams",
...
# 7/18 Fav3 lock: keep loudest ⭐ (esp. vs Littell 🧤) off Goblin 3-leg.
...
for _nm in (
    ...
    if not _r or _r["name"] not in FAVS:
        continue
    if _r["name"] in straight_names:
        continue
    if _r["name"] in {x["name"] for x in _forced_fav3}:
        continue
    if _r["split"] < -0.10 and not (_r["hr"] >= 2 and effective_park_pct(_r) >= 20):
        continue
    _forced_fav3.append(_r)
    if len(_forced_fav3) == 3:
        break
if len(_forced_fav3) == 3:
    fav3 = _forced_fav3
    fav3_names = {x["name"] for x in fav3}
    _rebuild_pool = sorted(
        [
            r
            for r in straight_rows
            if r["name"] not in straight_names
            and r["name"] not in fav3_names
            and goblin_top3_ok(r)
        ],
        key=hr_rank_sort_key,
        reverse=True,
    )
    top3 = pick_top_n(_rebuild_pool, 3, exclude_names=straight_names | fav3_names, max_per_game=2, max_per_team=2)
    if len(top3) < 3:
        _rebuild_relaxed = sorted(
            [
                r
                for r in straight_rows
                if r["name"] not in straight_names
        ...
        top3 = pick_top_n(
            _rebuild_relaxed,
            3,
            exclude_names=straight_names | fav3_names,
            max_per_game=2,
            max_per_team=2,
        )
    _two_excl = straight_names | fav3_names | {x["name"] for x in top3}
    _two_pool = sorted(
        [
            r
            for r in straight_rows
            if r["name"] not in _two_excl
            and r["name"] not in FAVS
            and goblin_hr_leg_ok(r)
        ],
        key=hr_rank_sort_key,
    ...
    if len(two_leg) < 2 and len(rows) <= 30:
        ...
        two_leg.extend(
            pick_top_n(
                thin_two,
                2 - len(two_leg),
                exclude_names=_two_excl | {x["name"] for x in two_leg},
                max_per_game=1,
                max_per_team=3,
            )
        )
...
def update_manifest():
    ...
    if "2026-07-17" in old:
        old["2026-07-17"] = {
            "date": "2026-07-17",
            "label": "July 17, 2026",
            "href": "archive/2026-07-17.html",
        }
    ordered = [
        {"date": SHEET_DATE, "label": "July 18, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-07-17",
        "2026-07-16",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Saturday, July 18, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
