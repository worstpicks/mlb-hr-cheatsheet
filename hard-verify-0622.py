#!/usr/bin/env python3
"""Hard verify June 22 sheet: duplicates, counts, summary, straights, bums."""
...
if sheet_date != "2026-06-22":
    errors.append(f"sheet-date wrong: {sheet_date}")
if "Monday, June 22, 2026" not in text:
    errors.append("header weekday wrong: expected Monday, June 22, 2026")
...
stale_june20_only = [
    "Kody Clemens", "Endy Rodriguez", "Christian Walker", "Jeremy Pena", "Brandon Lowe",
    "Max Muncy", "Bryan Reynolds", "Tyler Callihan", "Gabriel Arias",
    "Tomoyuki Sugano", "Connelly Early", "Taj Bradley",
]
for stale in stale_june20_only:
...
if summary_start != -1 and summary_end != -1 and summary_end > summary_start:
    ...
    stale_summary = [
        "Kody Clemens", "Endy Rodriguez", "Christian Walker", "Jeremy Pena", "Brandon Lowe",
        "Max Muncy", "Bryan Reynolds", "Tomoyuki Sugano", "Tyler Callihan",
    ]
    for stale in stale_summary:
        if stale in summary_block:
            errors.append(f"stale prior-slate name in Goblin/summary block: {stale}")
    if "Dalton Rushing" not in summary_block and "Jackson Chourio" not in summary_block:
...
# Verify preview games block matches build output (not stale prior slate).
preview_titles = re.findall(r'title: "([A-Z]{2,3} @ [A-Z]{2,3})', text)
build_titles = [g["title"].split(" - ")[0] for g in build.games]
if preview_titles != build_titles:
    errors.append(f"preview games mismatch: {preview_titles[:3]}... vs {build_titles[:3]}...")
stale_in_preview = ["CLE @ HOU", "NYM @ PHI", "PIT @ COL", "LAA @ ATH", "MIN @ ARI"]
for stale_game in stale_in_preview:
    if any(t.startswith(stale_game) for t in preview_titles):
        errors.append(f"stale prior-slate game in preview: {stale_game}")

if errors:
    print("FAIL hard-verify-0622:")
    for e in errors:
        print(f"  {e}")
    raise SystemExit(1)

print("OK hard-verify-0622")
