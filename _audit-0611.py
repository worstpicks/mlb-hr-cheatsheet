#!/usr/bin/env python3
"""Comprehensive June 11 sheet audit."""
...
stale = ["Brandon Marsh", "Brent Rooker", "Max Scherzer", "Junior Caminero", "Nick Kurtz", "Zack Gelof"]
for s in stale:
    if s in text:
        errors.append(f"stale prior-slate name in preview: {s}")

patch_globals: dict = {"__file__": str(ROOT / "patch-0611-preview.py"), "__name__": "__main__"}
cut = (ROOT / "patch-0611-preview.py").read_text(encoding="utf-8").split("THREE_LEG_HR =")[0]
exec(compile(cut + "\n", "patch-0611-preview.py", "exec"), patch_globals)
