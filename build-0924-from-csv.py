#!/usr/bin/env python3
"""Build 2026-09-24 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-24"
OUT = ROOT / "build-sheet-2026-09-24.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0924-matchups.py. Nothing is carried today: every arm the sheet
...
PITCHER_HAND = {
    # 2026-09-24: Cantillo (PRIM) replaces the opener Espino for Cleveland.
    "Joey Cantillo": "L",
    # 2026-09-24: Fedde (PRIM, over the opener Hudson) and the two game-two arms.
    "Erick Fedde": "R",
    "CJ Van Eyk": "R",
    "Trey Gibson": "R",
    # 2026-09-24: Bradford replaces the opener Alexander (see PROBABLE_OVERRIDES);
...
def load_pitcher_workload(games_csv: dict[str, dict]) -> dict[str, float]:
    ...
    for full in sorted(names):
        try:
            ...
            if pid is None:
                # PropFinder writes "Zach Thornton", MLB "Zac Thornton": same surname, a
                # keystroke apart on the first name. The exact-name lookup missed him and
                # his K line fell back to the league start. Match the way the research
                # tab already does rather than giving up on the schedule id.
                from research.mlb_api import _same_person_diff_spelling

                near = [i for n, i in probable_ids.items()
                        if _same_person_diff_spelling(n, full, lambda x: name_lookup_key(x).replace(" ", ""))]
                if len(near) == 1:
                    pid = near[0]
            if pid is None:
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-24 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        ...
        "Alejandro kirk", 2026-09-24 sent "Alejanro Kirk", which matched nothing at
