#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-11. Does not commit or push."""
...
spec = importlib.util.spec_from_file_location("build0811", ROOT / "build-sheet-2026-08-11.py")
...
# 8/11 judgment: O0.5 Bell vs Young (slate-best zone/attack lane).
# O1.5 Caminero vs Barnett 🧤 (best multi + park/bum; Pederson reserved for Fav3).
# Reject Siri O1.5 — 0/0 pitcher lane vs Bradford.
_bell = next((r for r in straight_rows if r["name_plain"] == "Josh Bell"), None)
_caminero = next((r for r in straight_rows if r["name_plain"] == "Junior Caminero"), None)
if _bell is not None:
    straight_o05 = _bell
if _caminero is not None and _caminero["game_key"] != straight_o05["game_key"]:
    straight_o15 = _caminero
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/11 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Ben Rice", "Andres Chaparro", "Lawrence Butler"]
_two_lock_names = ["Brady House", "JJ Bleday"]
_fav3_lock_names = ["Joc Pederson", "Pete Alonso", "Dylan Crews"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 11, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-10",
        "2026-08-09",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Tuesday, August 11, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
