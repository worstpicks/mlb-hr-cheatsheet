#!/usr/bin/env python3
"""Synthesize the hr-matchups games PropFinder did not export for 2026-08-18.

Three starters changed after PropFinder built its exports, all confirmed against
the MLB Stats API (Kansas City and Chicago already show their arm in the boxscore):

  ATH  Jack Perkins -> Brady Basso
  KC   Mason Black  -> Daniel Lynch IV
  CWS  Erick Fedde  -> Bryan Hudson

Every 8/18 file -- matchups, weak spots, zone, pitcher summary, HR risk -- was
built around the superseded arms, so without this the listed bats in ATH @ KC and
CWS @ CHC would be scored against pitchers who are not throwing.

Nothing here is invented. The lineups and batter form (and the real listed odds)
are copied from the same-day sibling export for that game -- same rosters, and
batter form is pitcher-independent. Pitcher blocks are carried from each arm's
most recent real export where one exists, and left blank where none does so the
sheet renders the honest "no MLB HR data yet" lane.
"""
...
GAPS: dict[str, dict] = {
    # Perkins is the ATH arm in the stale export, so his batter rows are the KC lineup.
    "Brady Basso": {
        "matchup": "ATH @ KC",
        "pteam": "ATH",
        "oteam": "KC",
        "lineups": "sibling",
        "sibling": f"hr-matchups-ATH-at-KC-Jack-Perkins-{DATE}.csv",
        "block_from": None,  # no matchup export for Basso on any prior slate
    },
    # Black is the KC arm in the stale export, so his batter rows are the ATH lineup.
    "Daniel Lynch IV": {
        "matchup": "ATH @ KC",
        "pteam": "KC",
        "oteam": "ATH",
        "lineups": "sibling",
        "sibling": f"hr-matchups-ATH-at-KC-Mason-Black-{DATE}.csv",
        "block_from": "hr-matchups-KC-at-LAD-Daniel-Lynch-IV-2026-08-12.csv",
    },
    # Fedde is the CWS arm in the stale export, so his batter rows are the CHC lineup.
    "Bryan Hudson": {
        "matchup": "CWS @ CHC",
        "pteam": "CWS",
        "oteam": "CHC",
        "lineups": "sibling",
        "sibling": f"hr-matchups-CWS-at-CHC-Erick-Fedde-{DATE}.csv",
        "block_from": None,
    },
}

# Stale-probable exports: kept on disk as the lineup source above, dropped from the
# manifest so derive_games_from_csv does not see two arms per side.
SUPERSEDED: list[str] = [
    f"hr-matchups-ATH-at-KC-Jack-Perkins-{DATE}.csv",
    f"hr-matchups-ATH-at-KC-Mason-Black-{DATE}.csv",
    f"hr-matchups-CWS-at-CHC-Erick-Fedde-{DATE}.csv",
]
...
CARRIED_RISK_ROWS: list[dict] = [
    {
        ...
        "time": "7:40 PM",
        "vs": "vs",  # KC is home
        "source": "hr-targets-overall-2026-08-12.csv",
        "cells": ["-1.23", "-1.31", "-0.40", "0.41", "4.1%", "6.0%", "34.6%", "23.2%", "8.0%", "507"],
    },
    {
        "pitcher": "Brady Basso",
        "time": "7:40 PM",
        "vs": "@",  # ATH is away
        "source": "hr-targets-overall-2026-07-25.csv",
        "cells": ["-0.54", "-1.06", "-0.31", "0.46", "9.0%", "5.3%", "47.5%", "21.5%", "8.3%", "130"],
    },
]
