#!/usr/bin/env python3
"""Patch preview sheet to 2026-06-17. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-06-16.html"
SHEET_DATE = "2026-06-17"

spec = importlib.util.spec_from_file_location("build0617", ROOT / "build-sheet-2026-06-17.py")
...
# 6/17 judgment: O1.5 Kurtz (+38% park, 5 HR) + O0.5 Schwarber (+32% park, +0.47 split).
rows_by_plain_early = {r["name_plain"]: r for r in rows}
_j_o15 = rows_by_plain_early.get("Nick Kurtz")
_j_o05 = rows_by_plain_early.get("Kyle Schwarber")
if (
    _j_o15
    and _j_o05
    and _j_o15["hr"] >= 2
...
# 6/17 Favorite 3-leg judgment: loud favorites with usable lanes (no Colt Keith -1.01).
_fav3_pick_order = ["Heriberto Hernandez", "Matt McLain", "Jose Siri", "Mookie Betts"]
...
for _nm in _fav3_pick_order:
    ...
    if _r["name"] in {x["name"] for x in top3} or _r["name"] in {x["name"] for x in two_leg}:
        continue
    if not fav3_lane_ok(_r) and not (
        effective_park_pct(_r) >= 10 and (_r["hr"] >= 2 or _r["near"] >= 3)
    ):
        continue
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "June 17, 2026 — current slate", "href": "index.html"},
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Wednesday, June 17, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
