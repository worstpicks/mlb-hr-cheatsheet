#!/usr/bin/env python3
"""Fill hr-matchups PropFinder did not export for 2026-09-12.

  Nothing to fill. All 15 games shipped both arms -- 30 hr-matchups files and 30
  pitcher-weak-spots files -- so this run is a no-op that exists to prove it. If a
  later export goes missing, the gap goes in GAPS and is filled from that arm's
  most recent real start rather than from a league default.
"""
...
GAPS: dict[str, dict] = {
    # Kyle Harrison's export saved before its batter table loaded: the pitcher block
    # is there and complete, the Cincinnati hitters are not, and the zone feed skipped
    # him too. Five bats on today's prop list are in that lineup, so the side cannot
    # simply be dropped. The order and handedness come from his own weak-spots export
    # (today), the pitcher block is kept verbatim from the broken file (today), and
    # batted-ball form is carried from each hitter's most recent real export -- form
    # is a property of the hitter, not of who is pitching. ZONE is left blank because
    # zone fit IS a property of the arm and nothing measured it for this one.
    "Kyle Harrison": {
        "matchup": "CIN @ MIL",
        "pteam": "MIL",
        "oteam": "CIN",
        "lineups": "weakspots",
        "block_from": f"hr-matchups-CIN-at-MIL-Kyle-Harrison-{DATE}.csv",
        "force": True,
    },
}
