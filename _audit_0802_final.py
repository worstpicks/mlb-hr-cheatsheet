#!/usr/bin/env python3
"""Final local audit for 2026-08-02 (do not push)."""
...
def main() -> int:
    ...
    if "August 2, 2026 — current slate" not in html:
        fail("manifest missing August 2 current slate")
    ...
    if not m or int(m.group(1)) != 90:
        fail(f"expected 90 props, got {m.group(0) if m else None}")
    ...
    # 8/2 targets: only Max Scherzer clears BUM_RISK_MIN (0.95).
    if len(bums) < 1:
        fail(f"expected at least 1 bum, got {bums}")
    if not any(p == "Max Scherzer" for p, _ in bums):
        fail(f"Max Scherzer should be bum, got {bums}")
    ...
    if 'content="2026-08-01"' not in arch.read_text(encoding="utf-8"):
