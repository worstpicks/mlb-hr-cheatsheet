#!/usr/bin/env python3
"""Patch preview sheet to 2026-07-19. Does not commit or push."""
...
ARCHIVE_PREVIOUS = ROOT / "preview" / "archive" / "2026-07-18.html"
SHEET_DATE = "2026-07-19"

spec = importlib.util.spec_from_file_location("build0719", ROOT / "build-sheet-2026-07-19.py")
...
# 7/19 Fav3: data-driven from user ⭐ (straights excluded upstream).
...
def update_manifest():
    ...
    if "2026-07-18" in old or (ARCHIVE_PREVIOUS.is_file()):
        old["2026-07-18"] = {
            "date": "2026-07-18",
            "label": "July 18, 2026",
            "href": "archive/2026-07-18.html",
        }
    ordered = [
        {"date": SHEET_DATE, "label": "July 19, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-07-18",
        "2026-07-17",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Sunday, July 19, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
