#!/usr/bin/env python3
"""Build 2026-09-05 sheet from imported CSVs and user prop list."""
...
DATE = "2026-09-05"
OUT = ROOT / "build-sheet-2026-09-05.py"
BUM_RISK_MIN = 0.95

# Arms whose HR-risk row was carried from an earlier slate by
# build-synthetic-0905-matchups.py. Nothing is carried today: every arm the sheet
...
# Two Max Muncys are exported today: Oakland's right-handed infielder (in the ATH
# bench-inclusive export, NOT in their posted lineup) and the Dodgers' left-handed
# third baseman batting fourth. batter_ctx is last-write-wins over alphabetically
# sorted games, so without this pin the prop would silently resolve to whichever
# game sorted last. The owner's list places him among Dodgers bats, and only the
# LAD one is starting.
...
def main() -> int:
    ...
    # ships the prop list's typos: 2026-09-05 arrived with "Alejandro kirk". When the
    ...
    def canonical_case(name: str) -> str:
        exported = export_case.get(name_lookup_key(name))
