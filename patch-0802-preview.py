#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-02. Does not commit or push."""
...
SHEET_DATE = "2026-08-02"

spec = importlib.util.spec_from_file_location("build0802", ROOT / "build-sheet-2026-08-02.py")
...
# 8/2 judgment: O0.5 Crooks (LHB +2.20 vs Scherzer 1.96 🧤) · O1.5 Castro (3 HR / 4 near at COL).
# Murakami leads raw multi/EV but fails matchup support (Jax risk −0.22 / tiny split) — skip for O1.5.
_crooks = next((r for r in straight_rows if r["name_plain"] == "Jimmy Crooks"), None)
_castro = next((r for r in straight_rows if r["name_plain"] == "Willi Castro"), None)
if _crooks is not None:
    straight_o05 = _crooks
if _castro is not None and _castro["game_key"] != straight_o05["game_key"]:
    straight_o15 = _castro
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/2 Goblin judgment: stack loudest attack lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Pete Crow-Armstrong", "Vaughn Grissom", "Endy Rodriguez"]
_two_lock_names = ["Nick Kurtz", "Lane Thomas"]
_fav3_lock_names = ["Alec Burleson", "Salvador Perez", "Taylor Trammell"]
...
def update_manifest():
    ...
    if "2026-08-01" in old or ARCHIVE_PREVIOUS.is_file():
        old["2026-08-01"] = {
            "date": "2026-08-01",
            "label": "August 1, 2026",
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 2, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-01",
        "2026-07-31",
...
def patch_preview(manifest):
    ...
    text = re.sub(
        ...
        "<p>Sunday, August 2, 2026 — Worst Pickz HR cheat sheet",
        text,
        count=1,
    )
