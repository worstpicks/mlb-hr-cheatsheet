#!/usr/bin/env python3
"""Patch preview sheet to 2026-08-07. Does not commit or push."""
...
SHEET_DATE = "2026-08-07"

spec = importlib.util.spec_from_file_location("build0807", ROOT / "build-sheet-2026-08-07.py")
...
# 8/7 judgment: O0.5 Contreras vs Perkins 🧤 (+0.37 / 0.96, Fenway +11).
# O1.5 Burleson⭐ vs Feltner 🧤 (+1.21 / 1.36, 2 HR / 2 near) — different game.
_contreras = next((r for r in straight_rows if r["name_plain"] == "Willson Contreras"), None)
_burleson = next((r for r in straight_rows if r["name_plain"] == "Alec Burleson"), None)
if _contreras is not None:
    straight_o05 = _contreras
if _burleson is not None and _burleson["game_key"] != straight_o05["game_key"]:
    straight_o15 = _burleson
straight_names = {straight_o05["name"], straight_o15["name"]}
...
# 8/7 Goblin judgment: loudest distinct lanes after straights; reserve top ⭐ for Fav3.
_top3_lock_names = ["Jesus Sanchez", "Mickey Moniak", "Jackson Chourio"]
_two_lock_names = ["Jimmy Crooks", "Wilyer Abreu"]
_fav3_lock_names = ["Owen Caissie", "Bryce Harper", "Rhys Hoskins"]
...
def update_manifest():
    ...
    ordered = [
        {"date": SHEET_DATE, "label": "August 7, 2026 — current slate", "href": "index.html"},
    ]
    for date in [
        "2026-08-06",
        "2026-08-05",
