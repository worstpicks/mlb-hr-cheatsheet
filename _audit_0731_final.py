#!/usr/bin/env python3
"""Final local audit for 2026-07-31 (do not push)."""
...
def main() -> int:
    ...
    if "Friday, July 31, 2026" not in html:
        fail("hero must be Friday, July 31, 2026")
    if "Thursday, July 30, 2026" in html or "Wednesday, July" in html:
        fail("stale weekday/date in hero")
    if "July 31, 2026 — current slate" not in html:
        fail("manifest missing July 31 current slate")
    if "July 30, 2026 — current slate" in html:
        fail("7/30 still labeled current")

    m = re.search(r"(\d+) listed HR props", html)
    if not m or int(m.group(1)) != 87:
        fail(f"expected 87 props, got {m.group(0) if m else None}")
    ...
    if not arch.exists():
        fail("7/30 archive missing")
    if 'content="2026-07-30"' not in arch.read_text(encoding="utf-8"):
        fail("7/30 archive wrong sheet-date")
