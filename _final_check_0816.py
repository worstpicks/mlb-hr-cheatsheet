#!/usr/bin/env python3
"""Final pre-push verification for the 2026-08-16 sheet.
...
def main() -> int:
    ...
    chk("root carries the 8/16 slate date", 'content="2026-08-16"' in root)
    ...
    prev = (ROOT / "preview" / "archive" / "2026-08-15.html").read_text(encoding="utf-8")
    missing = sorted(secs(prev) - secs(cur))
    chk("no sections lost vs 8/15", not missing, str(missing))
    for f in [
        "Homerun Form",
        "Damage Window",
